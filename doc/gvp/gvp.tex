...
\newunicodechar{▮}{\hspace{0.12pt}|\hspace{0.12pt}}
...
\textit{Scratch} \parencite{Resnick2009Scratch} claim to have over 135
million registered users as of 2024
\parencite{ScratchFoundationAnnualReport2024}.
...
automation, are also covered by visual programming tools like Simulink
\parencite{Simulink2025}, Unreal Engine Blueprint
\parencite{UEBlueprints2025} or LabVIEW \parencite{LabVIEW2025}.
...
programming languages is unquestionable \parencite{TIOBEIndex2025}.
...
the Racket programming system \parencite{Felleisen2018}, developed by the
PLT group, although even the UNIX operating system, whose components
are known to employ parser generators such as TMG or YACC, is also
sometimes considered a Language-Oriented Programming
environment \parencite{Kernighan2019}.
...
Racket programming system with programming education in
mind \parencite{Findler2010_DrRacket}. The Racket itself is derived
from the Scheme programming language, so it inherits its
\textit{heavily-parenthesized} syntax of
...
Programming system \parencite{Flatt2006SchemeWithClasses}.
...
\textit{Universe} \parencite{Felleisen2htdpUniverse}. Racket has also
been used as the core of an artistic live-programming environment
called \cite{Fluxus}.
...
The paper \textit{Adding Interactive Syntax to Textual
  Code} \parencite{Andersen2020} describes a principled attempt to turn
dr Racket into a General Visual Programming environment. It dwells on
the notion of code execution phases that were specified for the Racket
programming language (which allows for arbitrary code execution during
compilation time for the purpose of macro expansion) -- so for example,
there is a syntax expansion phase, as well as runtime execution phase.
...
Leif Andersen's PhD thesis, \textit{Adding Visual and
  Interactive-Syntax to Textual Programs} \parencite{Andersen2022}. In
the course of her work, she concluded that the Racket's GUI library
(used, among other things, to build the Dr Racket IDE) isn't flexible
enough to be reused inside the domain-specific editors.
...
(cf. figure \ref{fig:rbtree-visr}).

\begin{figure}[!h]
  \centering%
  \includegraphics[width=1.0\linewidth]{images/rbtree-visr}\hfill%
  \caption{A definition of Okasaki's Red-Black Tree balancing
    algorithm that uses Andersen's Visual Interactive Syntax running
    in dr Racket}
  \label{fig:rbtree-visr}
\end{figure}
...
The exact definition of the \texttt{g/let} form in ClojureScript is
the following \parencite{Andersen2024}:
...
in a principled manner \parencite{Omar2016Hazelnut}. Unlike Andersen,
who focuses on adding visual extensions to textual code, the makers of
Hazel decided to create a structure editor in such a way that the
experience of using it resembles textual programming -- but at every
moment of program development the program code is properly structured
(even if it contains holes).
...
\textit{livelits}, which are user-defined visual
extensions \parencite{Omar2021LiveLits}.
...
school grade cutoffs (see figure \ref{fig:hazel-livelits}), and
another specialized widget for adjusting image filters (brightess and
contrast).

\begin{figure}[!h]
  \centering%
  \includegraphics[width=1.0\linewidth]{images/hazel-livelits}\hfill%
  \caption{Examples of \textit{livelits} in the Hazel programming system}
  \label{fig:hazel-livelits}
\end{figure}


However, the examination of the code base shows that most of those
livelits are built-in (implemented in ReasonML), and therefore do not
use the extension system described in the paper. The only example of a
livelit defined and used from Hazel itself available for examination
is a variant of slider named \texttt{\$slidy}. However, the exact
extension method differs from the one described in the paper, and it
resorts to techniques invoking inline JavaScript code.
...
Another General Programming System to consider is
Polytope \parencite{Evans2025Polytope}. Polytope is an editor written
in TypeScript and running in the browser, which also provides
fundamental architecture.  The current implementation of Polytope is
designed to work with JavaScript code.  It comes with a bunch of
specialized editors, including editors for directed and undirected
graphs, math formulas, markdown and music staff. It should in
principle be possible for developers to define their own editors and
use them post hoc, but there is no distinct automatic mechanism for
scanning and installing definitons in the system.
...
\begin{figure}[!h]
  \centering%
  \includegraphics[width=1.0\linewidth]{images/polytope-examples}\hfill%
  \caption{Some specialized editors that are available in the Polytope
    editor.}
  \label{fig:polytope-examples}
\end{figure}
...
the assumption to base the editing environment around text, rather
than structure editing, is very much like in the Interactive Visual
Syntax project.

\subsection{JetBrains MPS}

JetBrains MPS (MetaProgramming System) is a \textit{language
  workbench} for creating domain-specific programming environments
developed by the same company that develops and maintains numerous
Integrated Development Environments for various programming systems,
including IntelliJ IDEA, Android Studio, CLion, PyCharm, PhpStorm,
WebStorm and a few others. The company also created the Kotlin
programming language. Despite being developed by a commercial software
company, MPS is available with full source code under the Apache
License.  It runs on the JVM platform.

MPS is a complex piece of software \parencite{JetBrainsMPS_UserGuide}.
One of its important components is a highly tweakable
\textit{projectional editor} which is able to operate directly on the
edited program's abstract syntax tree, even though the process is made
to resemble working with text. For this purpose, the editor has to
understand the structure of a programming language that it's supposed
to edit.
...
state machine definitions in the \textit{mbeddr}
project \parencite{Voelter2017_mbeddr}.
...
\parencite{nierstrasz2024moldabledevelopmentpatterns}. It is based on Pharo
...
Programming system. Conversely, projects such as eToys
\parencite{Kay1997_EtoysSimStories, Kay2007_ChildrenLearningByDoing}
or Sandblocks \parencite{Beckmann2020VisualProjectionalEditor} have
proven that Smalltalk-based systems offer a lot of flexibility when it
comes to non-standard program representations.
...
such as \textit{undo} or \textit{copy-paste} \parencite{Godek2023}.
...
buttons, a visual stepper \parencite{Godek2024} and a simple
two-dimensional physics engine (which is capable of using the phone's
accelerometer for accessing gravity information).
...
analyzing various existing GVP systems. However, we did not explain
why such systems might be worth pursuing.
...
enhance the clarity of the source code of their programs.
...
The difference between the definition of \texttt{f} and its example
usage is that the first one is \textit{abstract} and introduces a new
concept into the system, while the second is \textit{concrete} and it
only puts certain content in front of the mind's eye.
...
because its domain is naturally expressed in text. However, there are
...
Similarly, consider the following example of the \textit{A*
...
This example is quite complex; even understanding what it is about
requires effort. It would be much better if we could just see the
graph and the path on the graph -- or better yet, to see how the
algorithm operates step by step.
...
difficult to use than conventional text-based systems.
...
because it makes the tiles resemble comb teeth, but also because it
represents \textit{combinations} of expressions.)
...
defined by users could be significantly improved -- with regard to
...
representation already works like a sort of an indentation, and the
code that uses the traditional Lisp-style indentation looks
\textit{too indented} in GRASP.
...
So far we've been mainly considering the \textit{close-up} perspective
on programming -- which is the usual perspective when people discuss
programming tools. But the pinch zoom that is present in GRASP also
provides a fairly unusual way of looking at code -- it allows to
smoothly zoom out to see entire documents at once. Unfortunately,
these zoomed-out views are not very informative: they typically appear
as vertical white stripes with some illegible dark spots on them.

This is particularly frustrating when we think that it is possible to
construct a \textit{bird's eye view} of a program module that would
actually be useful, and that such view could contain a class diagram
or function signatures, devoid of their actual implementations.

And when we zoom out even further, we should no longer see a single
module, but instead we should see the modules from the entire project.

Currently this is usually done by means of a file browser. But this
representation usually shows a module as a generic icon with a file
name, residing in a directory tree. While this representation gives
some information, there is a lot of information that it misses.  In
particular, the size of the icon could depend (logarithmically or
otherwise) on the size of the module. Moreover, in module systems that
xpreclude circular dependencies, the modules that make up a project
naturally form a layered graph, where the topmost modules are the ones
that have no dependencies, and the bottommost ones usually contain
application-related code. It would also be desirable to be able to
highlight all modules that a given module depends on, as well as all
the modules that depend on a given module (but in order to remain
legible, this visualization would need to be dynamic)\footnote{This
idea is reminescent of the \textit{C4 model} developed by Simon Brown
\parencite{Brown_SAfD}, as well as the demo from Ivan Daniluk's 2019
GopherCon Europe talk titled \textit{Rethinking Visual Programming}
\parencite{Daniluk_GopherCon2019}.}.

Another thing that might be important for the adaption of the concepts
presented here is the availability of systems which support them.

In particular, integrating GRASP with web browsers would make it
possible to have GRASP snippets (including extensions such as its
visual stepper) available on web pages.

There is also a number of smaller things that could be added to or
improved in the system. For example, it could be handy to have a
generic object inspector, which would allow to add arbitrary Java
objects to the document.  Another nice feature would be an integration
of GRASP with a popular version control system such as git, which also
has a great potential of interesting and valuabe
visualizations\footnote{There is a number of code quality assessment
techniques developed by Adam Tornhill that were inspired by forensic
analysis \parencite{Tornhill2015CrimeScene}}. 

But, in order for those developments to make sense, GRASP must first
become sufficiently usable and robust to acquire a user base.
