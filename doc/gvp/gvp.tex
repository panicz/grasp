...
This section is based mainly on our developments of the fourth
prototype of GRASP, but it describes architectural decisions that we
believe are going to be preserved in all future versions of GRASP.

The most fundamental component of GRASP is called \texttt{Element}.
\texttt{Element}s are fundamentally of two kinds -- they either are
\texttt{Space}s or \texttt{Tile}s. The main difference between them is
that \texttt{Tile}s are rectangular (so they can be characterized by
their width and height), whereas \texttt{Space}s are not.

What all \texttt{Element}s have in common is that they can be used to
move the cursor's index forward or backward and to fetch the element
under the cursor's index. They can also be rendered in the document.

GRASP comes with a number of primitive implementations of the
\texttt{Tile} interface -- most notably the ones that represent boxes,
atoms, strings and block comments.

Users can also define their own implementations of the \texttt{Tile}
interface by means of its \textit{extension mechanism}. User-defined
extensions provide their own rendering procedure, and they are
typically \texttt{Interactive}, which means that the are able to
respond to user input (i.e. key presses and mouse/pointer events).
The aggregation of the \texttt{Interactive} and \texttt{Tile}
interfaces is called \texttt{Enchanted}.

There are fundamentally two ways of defining extensions: the first one
requires defining all the required methods (for drawing and input
handling). The second allows to define extensions from simpler
extensions by means of \textit{combinators}.

Figure \ref{fig:player-with-controls} shows the definition of
\texttt{PlayerWithControls}, which uses three combinators
(\texttt{bordered}, \texttt{beside} and \texttt{below}). This
definition is in turn used to define a \texttt{Stepper} widget, whose
instance is shown on figure \ref{fig:visual-stepper}.
    ...
    (beside
     ...
     (Button label: "▶▶▮"
	     action: (lambda ()
		       (player:fast-forward!)))))))
  \end{Snippet}
  \caption{An example usage of combinators (\texttt{bordered}, \texttt{beside},
    \texttt{below}) in the definition of another combinator}
