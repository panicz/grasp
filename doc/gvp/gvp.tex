...
a few others. Despite being developed by a commercial software company,
MPS is available with full source code under the Apache License.

MPS is a complex piece of software. One of its important components is
the \textit{projectional editor} which is able to operate directly on
the edited program's abstract syntax tree, even though the process
resembles working with text. For this purpose, the editor must
...
The definition of a language in MPS consists of a few \textit{aspects}.
The most fundamental type is the language's \textit{structure}, which
consists of a set of language units (the aforementioned \textit{concepts}
or \textit{syntax tree node types}). 
