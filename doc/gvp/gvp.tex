...
right soil in order to take root and flourish.

The single most important feature that a system must possess in order
to become succssful, is that it must address some of the needs of its
potential users.  It must also be simple enough to be understandable
by users, or at least it must provide some learning path that
eventually leads to sufficient understanding.
...
Programming system. It is neither a text-based system (like Polytope
or Visual Interactive Syntax) nor a projectional editor (like MPS or
Hazel). Instead, it is a generic structure editor for s-expressions,
so while it allows to create malformed programs (or rather
structures), the textual representation of those structures
necessarily consists of balanced parentheses.

GRASP renders a balanced pair of parentheses as a \textit{box}.

For example, here's what a definition of the \textit{factorial}
function in the Scheme programming language looks like when rendered
using the GRASP representation:
...
The boxes in GRASP occupy a rectangular area. The left parentheses
can be used for dragging a given box out of the document
and moving it elsewhere, and the right parentheses -- for resizing
the box.

GRASP also allows users to define \textit{extensions} or \textit{magic
  boxes}, which are essentially an incarnation of the domain-specific
editors discussed in the earlier section.

\subsection{A Brief History}

At the time of writing, GRASP went through four iterations of
development. The first prototype was written in Racket, and its
development was preceded by announcing a \textit{draggable rectangle
  challenge} on the Quora social
network{\url{https://eidolon-language.quora.com/Draggable-rectangle-challenge-part-I-the-introduction}}, which consisted of three tasks:
...
The purpose of the challenge was to explore a number of alternative
designs when it comes to the fundamental mechanics of GRASP editing.

The particular tension was between functional programming (immutable)
designs and object-oriented (mutable) designs.

The initial implementation in the Scheme programming language explored
the technique of \textit{mix-in composition}, where all the features
were implemented as mix-ins. This technique, however, turned out to be
problematic for several reasons.

\subsection{The Architecture of GRASP}

\subsection{Hardships and Design Mistakes}
