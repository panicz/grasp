...
No to wyglada na to, ze bysmy teraz chcieli sprobowac
zrobic cos na przegladarke?

Czy moze bysmy jeszcze pocisneli te nasze makra?



* 24.04.2025

Mozemy albo kontynuowac analize tego bledu ze zmiana rozmiaru
(co w sumie byloby pozyteczne), albo dalej probowac ukladac
"duzy obraz", czyli z grubsza Comb, POORMANS, makra.

Zasadniczo tez moglibysmy sprobowac zaimplementowac Comb
juz teraz w GRASPie. No ale wrocmy do tego naszego bledu.
...
No to teraz musimy sobie zobaczyc w jaki sposob dochodzi
do wywolania tej metody =press!=. Oczywiscie, w zaleznosci
od klientow, szczegoly beda roznic sie od siebie, ale
finalnie na pewno zostanie wywolany =ActualScreen:top:press!=

No dobra, to jakos psim swedem udalo sie ten blad naprawic.

I co dalej?

Na pewno mamy te wszystkie czarodziejstwa z pozycjami kursorow.

No i mamy te scroll bary do zrobienia.

Ale wydaje sie, ze swego rodzaju priorytetem jest uruchomienie
GRASPa w przegladarce

* 23.04.2025

na rzazie plan jest taki, zeby:
- zrobic kurs Hemanna (w emacsie)
- odtworzyc opcje impansji makr
- pracowac nad Comb i POORMANS

Nozemy tez sprobowac:
- naprawic obracanie przedzielonego edytora
- zoptymalizowac renderowanie na duzym zoomie

No to dajmy na to ze sie zajmiemy tym obracaniem.

W tej chwili jest ono realizowane z poziomu
=DocumentEditor:press!= za pomoca metody
=editor:transform:stretch!=

* 22.04.2025

Sprobujmy wydobyc z pracy magistetskiej nasz makroekspander,
i moze scalic go z ewaluatorem GRASPa?

No bo ogolnie wyglada na to, ze nie ma co liczyc na ewaluator
Kawy na Androidzie.

Moze bylby jakis sens w tym, zeby napisac cos, co by nam
przekladalo nasz nadzbior podzbioru Scheme'u na Fusion?
...
No dobra, to widzimy ze mamy fundamentalne problemy z reprezentowaniem
rzeczy. Tzn. ze np. zamiast zbiorow uzywamy list.

Jutro bedziemy kontynuowac prace nad tym ekspanderem/impanderem
(szkoda ze stracilismy dysk z laptopa - chyba ze nie?). Ale tak ogolnie
to byloby super gdyby nasz jezyk dawal nam prawdziwe zbiory (tak jak
clojure czy nawet to cosmy sobie w kawie zrobili)

Jednak powraca pytanie: co z tym Hemannem nieszcsesnym?

Opcje sa takie, zeby:
- zrobic tutorial w GRASPie na kompie
- napisac nowy ewaluator dla Kawy
- zrobic w Emacsie w Termuxie

No i jeszcze taka wpadla mysl do glowy, zeby sprobowac uzyc
=Comb= jako =syntax-object=, a przynajmniej zeby napisac
ewaluator dla =Comb=

I kolejna mysl, zeby w POORMANS kazdy obiekt mogl byc
drugim argumentem dla =eval=.

* 20.04.2025

jutro wrzucimy zrodla jsssssssss na githuba i zaktualizujemy
TODO, a pojutrze dodamy owijanie rzeczy w quote'y i fixa
w stepperze

a co dalej?

watki mamy w zasadzie trzy:
- telegrasp (comb, jsssssssss, poormans)
- hemann
- ksiazka

no dobra, to problem z Hemannem jest taki, ze ewaluator Kawy
na Androidzie dziala nieortodoksyjnie, i nie obsluguje makra
=match=.

No ale tak:
- chcielibysmy na pewno obsluzyc makto =match= rowniez
  w ewaluatorze
- ale jak to zrobic bazujac na samym jezyku Scheme?


Tutaj (oraz w przypadku =cond=) idea bylaby taka, zeby
od razu dopasowywac fragmenty wyrazenia do zmiennych,
i zastepowac je w ciele funkcji. Czyli jak mamy
...
i pattern =k= jest pierwszym pasujacym do wartosci =x=,
to w pierwszym kroku wszystkie pozoatale patterny znikaja,
i dostajemy
#+BEGIN_SRC racket
  (match x
    (k ...))
...
a dopiero w kolejnym kroku zastepujemy wystapienia komponentow
=k= odpowiednimi komponentami =x=, i zastepuhemy cale wyrazenie
cialem patternu.

* 19.04.2025

raczej bysmy chcieli cos w rodzaju

#+BEGIN_SRC racket
  (let ((x (take-expression!)))
    (put-expression! `(quote ,x)))
#+END_SRC

No dobra, to mamy juz jakas koncepcje.

Teraz bysmy jeszcze chcieli uzyc tych naszych
czarodziejstw do zaimplementowania TeleGRASPa.
...
#+BEGIN_SRC
  ...
  p - print
  ? - help
  /name/ - zwraca indeksy obiektow, do ktorych udaje sie
  dopasowac wzorzec
  0 3 - pozwala umiescic kusror na odpowiedniej sciezce
  0 3,0 5 - pozwala wybrac region
  l - wylistowuje otwarte dokumenty
  h - wyswietla historie edycji
  u - undo
  r - redo
  iwyrazenie - dodaje wyrazenie 
  d - usuwa biezacy obiekt
  n+ przesuwa kursor o n pozycji do przodu
  n- przesuwa kursor o n pozycji do tylu
  c 
...
No dobra, to sprobujmy moze zrobic tak, zeby wciskanie
` powodowalo owijanie biezacego wyrazenia quasiquotem,
a , - unquotem.

Jak by to mozna bylo zrobic:
1. przed aktualnym wyrazeniem umieszczamy
   np. (quasiquote )
...
2. przenosimy aktualne wyrazenie do srodka
...
3. zaczarowujemy nadrzedne wyrazenie

#+BEGIN_SRC racket
  (EnchantExpression at: Cursor := (the-cursor))
#+END_SRC

4. to wszystko powinno byc zaklete w sekwencje;

#+BEGIN_SRC racket
  (EditSequence operations: (list-of Edit))
#+END_SRC

Czyli biorac to wszystko do kupy:


#+BEGIN_SRC racket
  (define (wrap-expression! with: prefix ::Tile
                            at: cursor := (the-cursor)
...
No to jak teraz wyglada sytuacja?

Lista rzeczy, ktore wczesniej sprawialy trudnosc:
- pionowy ruch kursora
- wyszukiwanie tekstowe
- bledy zwiazane z ficzerami jezykowymi
  (tak jak to haszowanie mutowalnego obiektu)

Poza tym wydaje sie, ze schemat do indeksowania
wcisnietych klawiszy troche niepotrzebnie wchodzil
w droge podczas budowania pop-upow

 Kolejna rzecz, ze podczas renderowania moglibysmy
zbudowac drzewo binarne, zeby moc szybciej odwzorowywac
wspolrzedne ekranu we wspolrzedne dokumentu - co w sumie
moze miec znaczenie przy wiekszej ilosci obiektow
na ekranie

A tak naprawde to zalezy nam na odwzorowywaniu wspolrzednych
dokumentu we wspolrzedne edytora, a dopiero stamtad
przenosimy je na wspolrzedne ekranu.

Dodatkowo, ten sam dokument moze byc otwarty we wiecej
niz jednym edytorze, przez co odwzorowanie z dokumentu
na ekran jest w ogolnosci jeden-do-wielu.

No ale dobra. Fajnie. Swietnie. Ekstra.

Ale co robimy dalej?

Idealna, wysniona sciezka bylaby tak, ze:
- implementujemy jssssssss jako self-hosting
- implementuhemy POORMANS dla:
  - Kawy (to prawie juz mamy)
  - Racketa (bo wydaje sie nieodlegly)
  - jssssssss (bo powinnismy miec nad nim duza kontrole)
  - Chez (jako przystanek do innych Scheme'ow)
 - implementujemy GRASP od nowa w POORMANS
   - reprezentacja Comb
   - a co z TeleGRASP?
 - a jak juz tu dojdziemy, to co?

Trzeba:
- trzaskac rozszerzenia
- uzywac
- dokumentowac

No dobra, ale gdzies tam nam umykaja:
- gutenbergson
- UtWtC
- uzywanie tego, co zrobilismy do tej pory

Czyli tak:
- w niedziele tworzymy repozytorium dla fsssssss
- w poniedzialek wgrywamy tam projekt i commitujemy
  TODO

Poza tym jednak wydaje sie, ze powinnismy kontynuowac
prace nad:
- POORMANS
- Comb
- jsssssssss
...
javascript scrounged from a sophisticated superset of
a simple subset of scheme or something similar

* 16.04.2025

Wyobrazam sobie, ze =DocumentEditor= powinien byc w stanie
okreslic, ktory obiekt jest pierwszym widocznym obiektem.

Natomiast liste operacji edycji przerobilibysmy na interfejs.

#+BEGIN_SRC racket

  (define-interface Editable ()
    (move-expression! source :: Cursor
                      destination :: Cursor)
    :: Cursor

    (remove-expression-at! location :: Cursor)
    :: Cursor

    (insert-expression! expression :: Tile
                        location :: Cursor)
    :: Cursor
    
    (resize-box-at! location :: Cursor
                    width :: real
                    height :: real
                    anchor :: real)
    :: Cursor
    
    (insert-characters! characters :: (sequence-of char)
                        location :: Cursor)
    :: Cursor
    
    (remove-characters-before! location :: Cursor
                               amount :: int)
    :: Cursor
    
    (split-element-at! location :: Cursor
                       space :: Space)
    :: Cursor
    
    (merge-elements-removing-space-after! location :: Cursor)
    :: Cursor
    
    (atomically perform-edits::(maps () to: Cursor))
    :: Cursor
    )
...
#+BEGIN_SRC racket
  ...
  (define-object (Comb) :: Tile
    ...
    (define-private elements ::(sequence-of Tile)
      (FlexibleVector))
    
    (define-private spaces ::(sequence-of Space)
      (FlexibleVector (Space)))

    (define (first-index):: Index #\[)
    (define (last-index):: Index #\])

    (define (previous-index index):: Index
      (match index
        (0 #\[)
        (#\] (+ 1 (* 2 (length elements))))
        (index :: integer (max 0 (- index 1)))
        (_ index)))

    (define (next-index index):: Index
      (match index
        (#\[ 0)
        (index :: integer
               (let ((next (+ index 1)))
                 (if (is next < (+ 1 (* 2 (length elements)))) ;>
                     next
                     #\])))
        (_ index)))

    (define (part-at index):: Element
      (match index
        (,(first-index) (this))
        (,(last-index) (this))
        (index :: integer
    ...
    (define (key-typed key context):: boolean
      (let* ((cursor (the-cursor)))
        (match cursor
          (`(,@,context ,index)
           (assert (is index in (set #\[ #\])))
           (context:push index)
           ;; ???
           (context:pop index))
          
          (`(,@,context ,index ,top)
           (let ((child (that:part-at index)))
             (assert (eq? child (child:part-at top)))
             (context:push index)
             (edit-child child key context)
             (context:pop index)))

          (`(,@,context ,index . ,rest)
           (let ((child :: Element (that:part-at index)))
             (context:push index)
             (child:key-typed key context)
             (context:pop index))))))
    )

  (set! (Comb:keymap #\space) ...)
  (set! (Comb:keymap #\return) ...)
  (set! (Comb:keymap #\backspace) ...)
  (set! (Comb:keymap #\[) ...) 
  (set! (Comb:keymap #\]) ...)

  (define-object (Document):: EditableTile
...
#+BEGIN_SRC racket
  ...
  (Move from: Cursor
        to: Cursor
        with-shift: int := 0)
  (Remove element: (either pair EmptyListProxy)
          at: Cursor := (the-cursor)
          with-shift: int := 0)
  (Insert element: pair
          at: Cursor := (the-cursor))
  (ResizeBox at: Cursor := (the-cursor)
  			...
  			to: Extent
  			with-anchor: real)
  (InsertCharacter list: (sequence-of char)
                   after: Cursor := (the-cursor))
  (RemoveCharacter list: (sequence-of char)
                   before: Cursor := (the-cursor))
  (InsertComment content: TextualComment
                 at: Cursor := (the-cursor))
  (RemoveComment content: TextualComment
                 at: Cursor)
  (CommentExpression at: Cursor := (the-cursor)
                     with-shift: int)
  (UncommentExpression at: Cursor := (the-cursor))
  (SplitElement at: Cursor := (the-cursor)
                with: Space)
  (MergeElements removing: Space
                 after: Cursor := (the-cursor))
  (EnchantExpression at: Cursor := (the-cursor))
  (DisenchantExpression at: Cursor := (the-cursor))
#+END_SRC


No dobra, to teraz kwestia taka, ze mozemy:
- uporzadkowac operacje historyczne
- rozwazyc kwestie optymalizacji renderowania
  i wyliczania rozmiarow

* 13.04.2025

No dobra, to tym, co udalo sie zrobic w ciagu
kilku ostatnich dni, jest przede wszystkim
zalazek projektu /jssssss/, czyli JavaScript
Scrounged from a Simple and Straightforward
Subset of Scheme.

I teraz rodzi sie pytanie: co robimy dalej?

Byc moze moglibysmy zaczac od tego, zeby
odkopac kompilator z pracy magisterskiej.
...
no to mamy tak:
- jssssss
- system obiektow dla jezyka Scheme

No ale tak:
- zeby jssssss trzymal sie kupy, musimy dokonac ekspansji
  makr
- a przy okazji mozemy odwiedzic nasz stary kompilator
- i warto odswiezyc to, w jaki sposob "robi sie to"
  w Indianie
- wiec po raz kolejny sprobujemy zrobic tutorial Hemanna
  w GRASPie, co zapewne bedzie wymagalo paru zmian:
  - wcisniecie ' na obiekcie x powinno go zastepowac
    obiektem [quote x]
  - wcisniecie ` na obiekcie x powinno go zastepowac
    [quasiquote x]
  - wcisniecie , na obiekcie x powinno go zastepowac
    [unquote x]
  - wcisniecie (alt #\') na obiekcie [quote x]
    powinno go zastepowac x
  - itd.


No i tak: wydaje sie, ze dodanie takiego rozszerzenia
"owijajacego" jest dosc trudne, bo miesza sie nam
edycja dokumentu z edycja historii.

  
  
* 11.04.2025

na razie mamy takie zagadnienia:
- wyliczanie perfect hasha dla zbioru stringow
- czy chez scheme ma weak-key-hash-table?

no ale moze cofnijmy sie o krok. co chcemy teraz miec?
- GRASP ktory dziala w przegladarce

jak chcemy to osiagnac?
- zaprojektowac system obiektow kompilowany do:
- racketa/racketscripta
- kawy
- (a docelowo tez do cheza z SDLem)

A co z naszymi datomikowymi rozkminkami?
Na razie chyba nic (ale wyglada na to, ze docelowo
bedziemy chcieli napusac swojego)

No ok. To mozemy przeciez tez sprobowac uzyc racketscripta?

* 10.04.2025

No to teraz kolejna kwestia: co z mozliwoscia:
- dynamicznego dodania metody do interfejsu
- dynamicznego dodania metody do klasy

Na razie chyba mozemy sie tym nie przejmowac.

Natomiast w tej chwili musimy podjac taka decyzje
projektowa, ze do reprezentacji takich rzeczy, jak
klasy czy interfejsy, uzywamy wektorow.
...
#+BEGIN_SRC racket
  ...
  (define-syntax object
    (syntax-rules (::)
      ((object (arguments ...) :: interface
         spec ...)
       ;; musimy sprawdzic, czy specyfikacja jest zgodna
       ;; z interfejsem
       (let* ((members (only-names (visible-definitions spec ...)))
              ;; musimy sprawdzic, czy wszystkie te membery
              ;; przynaleza do interfejsu  i maja zgodne typy
              (member-accessor (perfect-hash members))
              (constructor (lambda (arguments ...)
                             spec ...
                             ;; zwracamy obiekt, czyli co?
                             ;; na pewno potrzebujemy czegos,
                             ;; co by nam odwzorowywalo nazwy
                             ;; w akcesory do specjalizacji.
                             ;; I tutaj mamy takie mozliwosci:
                             ;; - albo stworzymy w ramach klasy
                             ;; jakas haszmape, ktota bedzie nam
                             ;; odwzorowywac nazwy w indeksy,
                             ;; i za pomoca tych indeksow
                             ;; bedziemy sie odnosic do tablicy
                             ;; akcesotow, albo umiescimy
                             ;; akcesory bezposrednio w haszmapie.
                             ;;
                             ;; Za pierwszym rozwiazaniem przemawia
                             ;; to, ze takie rozwiazanie wydaje sie
                             ;; oszczedniejsze (i moze bylby sens
                             ;; stworztc w klasie doskonala funkcje
                             ;; haszujaca dla okreslonego zbioru
                             ;; danych)
                             )))
         constructor))))
...
no to tak:
- poniewaz klasa musi byc konstruktorem, to bedziemy ja
  utozsamiac z procedura
- musimy znalezc jakis spodob na trzymanie metadanych
  o procedurze
...
Musimy teraz wymyslic w jaki sposob bedziemy reprezentowac:
- klasy
- obiekty
- interfejsy

Interfejs musi miec przynajmniej tyle:
- tablice interfejsow nadrzednych
- liste metod
- odwzorowanie nazwy metody w jej sygnature typu

Klasa musi miec:
- wskazanie na interfejs
- wskazanie na klase nadrzedna
- zmienne i metody statyczne
- mozliwosc konstruowania instancji

Obiekt musi miec:
- wskazanie na klase
- mozliwosc wywolania metod oraz odczytu 

* 08.04.2025

POORMANS - Portable Object Orientation/Reflection
Module Across Numerous Schemes

#+BEGIN_SRC racket
  (define-syntax leftmost-atom
    (syntax-rules ()
      ((_ (left . right))
       (leftmost-atom left))
      ((_ atom)
  ...
  (define-syntax categorize-members
    (syntax-rules (define
                    define-private
                    define-static)
      
      ((categorize-members final params members)
       (categorize-members
        final
        params
        members
      ...
      ((categorize-members
       ...
       (categorize-members
        final params
        rest
        ((define (name . args) . body) . public-methods)
        public-fields
        private-methods
        private-fields
        static-methods
      ...
      ((categorize-members
       ...
       (categorize-members
        ...
        rest
        public-methods 
        ((define name . body) . public-fields)
        private-methods
        private-fields
        static-methods
        static-fields))
      ...
      ((categorize-members
        final params
        ((define-private (name . args) . body) . rest)
        public-methods
        public-fields
        private-methods
        private-fields
      ...
      ((categorize-members
       ...
       (categorize-members
        ...
        public-fields
        private-methods
        ((define-private name . body) . private-fields)
        static-methods
        static-fields))

      ((categorize-members
        final params
        ((define-static (name . args) . body) . rest)
        public-methods
        public-fields
        private-methods
        private-fields
       ...
       (categorize-members
        ...
        private-methods
        private-fields
        ((define-static (name . args)
           . body) . static-methods)
        static-fields))

      ((categorize-members
        final params
        ((define-static name . body) . rest)
        public-methods
        public-fields
        private-methods
        private-fields
       ...
       (categorize-members
        ...
        private-fields
        static-methods
        ((define-static name . body) . static-fields)))
    ...
    (syntax-rules ()
      ((build-class
       ...
       (let ()
         ...
         private-methods ...
         public-methods ...
         (setter-with-getter public-fields) ...
         (getter public-methods) ...))))

  (define-syntax define-object
    (syntax-rules (::)
      ((define-object (class-name . constructor-args)
                      :: interface
         . members)
       (categorize-members build-class (class-name
                                        interface)
                           members))))
...
No dobra, to wyglada na to, ze jeszcze troche bedziemy
sobie musieli pogaworzyc.

Definiowanie klasy bedzie
- wprowadzalo konstruktor, w ktorym bedziemy
  tworzyli wszystkie pola, a nastepnie gettery
  i settery dla tych pol

#+BEGIN_SRC racket
  (define (field-ref object field)
    ((getter object field)))

  (define (field-set! object field value)
    ((setter (getter object field)) value))

  (define-syntax object
    (syntax-rules (::)
      ((object args :: interface
         spec)
       (let ((class (new-class
                     'unnamed-class
                     (class-spec-superclass spec)
                     interface)))
         (lambda args
           (class-spec-fields+methods spec)
           (class-spec-initializer spec)
           (new-instance class (class-spec-accessors spec)))))))


#+END_SRC

* 06.04.2025 (no, prawie)

Teraz sprobujemy moze troche postudiowac to, jak dzialaja
w Rackecie te syntax-objecty

Tam ta struktura jest tak zdefiniowana
...
Wyglada na to, ze niewiele sie stad dowiemy. W Chez mamy z kolei
...
co jest wprawdzie prostsze, ale takze niezbyt pomocne

Wniosek z tego taki, ze moze na razie dajemy sobie z tym
spokoj?

No ale dobra, zalozmy ze tak.
I co wtedy?

Czy chcemy napisac translator z Eidolona do Scheme'u?
I czym jest typ?

Przede wszystkim, typ moze byc wbudowany (np. Javowy,
JSowy itd), albo zdefiniowany przez uzytkownika.

Po drugie, typ pozwala nam:
- stwierdzic, czy dany obiekt do niego nalezy
- skonstruowac nowy obiekt (na podstawie parametrow)
- zdekonstruowac zlozony obiekt na jego komponenty

Teoria kategorii mowi nam tyle, ze mamy dwa podstawowe
rodzaje typow:
- rekordy
- warianty

Natomiast Java poucza nas, ze przynajmniej niektore
typy albo sa interfejsami, albo implementuja interfejsy.


#+BEGIN_SRC scheme

(define-variants BaseType
  (Variant1 ...)
  (Variant2 ...)
  (Variant3 ...))
...
Wydaje sie, ze warto by bylo obejrzec sobie ten tutorial
Brady'ego, i ewentualnie przestudiowac The Little Typera.


* 05.04.2025

No dobra, to teraz mamy dwa tematy:
1. stworzenie jezyka w oparciu o Racket, ktory
   mialby czesc wspolna z podzbiorem Kawy uzytym w GRASP
2. nowa architektura GRASPa (tzn. nowa reprezentacja
   dokumentu, kursora, historii oraz operacji na
   dokumencie)

Mowiac skrotowo, chcemy miec taki =#lang= w Rackecie,
w ktorym bysmy mieli:

- ujednolicony =define-object= (poprawiony wzgledem tego,
co mielismy w Kawie)
- =define-record= (troche uproszczony wzgledem tego,
co mamy teraz w =define-type=, ale na swoj sposob tez troche rozbudowany)
- =define-interface= (zasadniczo taki jak mamy teraz)
- =a:b= => ($lookup$ a `b)
- =a::b= -> 3 tokeny: =a=, =::=, =b=
- =define-attribute=, =define-cache=, uogolnione settery

(natomiast po stronie Kawy chcielibysmy moze miec jakies wsparcie
dla =#lang=?)

- no i jeszcze generyczne wsparcie dla =Bundle=!


Moze tez powinnismy wrocic do takiego zagadnienia: czym jest typ?
- z jednej strony mozemy go utozsamiac z predykatem
- z drugiej moze z jakims generatorem? (tak jak w quickchecku)

A ten nasz =ObjectInspector= to by mniej wiecej tak wygladal:

#+BEGIN_SRC
┌────────────────────────────────────┐
│     java.class.Name@(hash)         │
├───────────────────────┬────────────┤
...
tak naprawde argumenty do metod powinny stanowic liste
do ktorej wartosci mozemy albo wybierac (jezeli to enum),
albo wpisywac z palca.


Zeby jednak do tego moglo dojsc, to byloby dobrze, zeby
dalo sie umieszczac edytor graspowych wyrazen wewnatrz
takiego pola

No i jeszcze powinnismy miec dla pol jeszcze jedna kolumne,
w ktorej bysmy wpisywali ewentualna nowa wartosc, oraz
przycisk "set", ktory pozwalaby nam to zmieniac (natomiast
dla metod powinnismy miec metode "call")


No dobra, natomiast co do tego Racketa, to pytanie,
czy nie chcielibysmy raczej stworzyc readera oraz
modelu obiektowego w czystym Schemie, i ewentualnie
tlumaczyc kodu do czystego Scheme'u, modelujac to
na tym, jak teraz dziala Kawa?


* 04.04.2025

No to jak na razie nasze wyobrazenie jest takie, ze:
- zaczniemy od zbudowania reprezentacji "Comb"
  (jako pary tablic) oraz "Loc" (jako tablicy
  indeksow)
- ale wczesniej wyodrebnimy i rozbudujemy
  system reprezentacji obiektow i rekordow
- sama architektura musi byc taka, ze:
  - rekurencyjnie przekazujemy wglab obsluge
    klawiszy

Aaaaa

no to teraz taka mysl, zeby napisac kod, ktory
bedzie jednoczesnie dzialal w Kawie i Rackecie,
tzn. zeby zbudowac kawowy jezyk dla Racketa.

(Jest tez pomysl na projekt Jacket Blake'a Shaw)

* 03.04.2025

wyglada na to, ze szybkosc ladowania datomika jest
ograniczona, co czyni go niepraktycznym do
zamierzonego zastosowania (choc docelowo moze
byloby warto - posilkujac sie ich architektura
- stworzyc szybsza implementacje?)

kolejnym krokiem byloby zatem wyprobowanie
bazy xtdb, ale tego raczej na szybko nie zrobimy

z tematow, ktore jakos tak materializuja sie
na horyzoncie, mamy:

- stworzenie rozszerzena =ObjectInspector=, ktore
  by wyswietlalo wszystkie publiczne wlasciwosci
  i metody inspektowanego obiektu

- wprowadzenie cache'u do renderowania

- zmiane reprezentacji dokumentu (i przepisanie
  wielu komponentow od nowa)

- udzial w spring lisp game jam z totalna kutasa 2d
  (integracja z dyn4j?)

Tak czy inaczej, wydaje sie, ze nalezaloby zaczac od
optymalizacji, tak zeby ten klient androidowy na powrot
stal sie uzywalny.

Ale zanim do tego dojdzie, musimy i tak przemyslec
architekture.

Wyjdzmy moze od tego, ze Clojure ma transactora, i ze
my tez wszystkich zmian chcielibysmy dokonywac przez
cos podobnego.

Tzn. zeby byl sobie pewien interfejs do edytowania
dokumentu.

Tyle ze 'jednosc' tego interfejsu jest trudna, jezeli
chcemy do tego, zeby edycja odbywala sie 'rekurencyjnie'

* 01.04.2025

No dobra, to dzisiaj bysmy chcieli polaczyc sie z
baza datomic-local z poziomu Kawy

W jaki sposob mialoby to wygladac?

Na pewno musimy:
- wywolac odpowiednie funkcje (owinac je?)
- skonstruowac odpowiednie struktury (czyli
  zbudowac cos, co by nam mapowalo kawowe
  listy na clojure'owe wektory)

No dobra, to teraz jeszcze musimy ustalic,
co jest nam potrzebne do pracy z baza danych.

No przede wszystkim adres tej bazy danych
(oraz ewentualnie dabe dostepowe jak login i haslo
albo sciezka dostepu do klucza prywatnego)

Podczas gdy w Clojure mamy do czynienia z takimi
bytami:
- =datomic.client.api:client= - zwraca (bezpolaczeniowy)
  obiekt reprezentujacy klienta bazy danych (np.
  datomic-local, datomic-cloud, datomic-pro)
- =datomic.client.api:connect= - zwraca obiekt
  reprezentujacy polaczeniez baza danych
- =datomic.client.api:db= - zwraca obiekt reprezentujacy
  aktualna wartosc bazy danych (niemutowalna) dla
  danego polaczenia

Zasadniczo wydaje sie zatem, ze jezeli chcemy miec
najnowsze dane, to powinnismy zawsze przed wykonaniem
zapytania pobrac nowy obiekt "db"
...
#+BEGIN_SRC scheme
  ...
  (define-alias Database datomic.client.api.protocols.Connection)

  (define-alias DatabaseSnapshot datomic.core.db.Db)

  (define-class VectorBundle (clojure.lang.PersistentArrayMap)
    ((*init* elements::(Object[]))
     (invoke-special clojure.lang.PersistentArrayMap (this)
    		   '*init* elements))
    ((toString)::String
  		   ...
  		   (map (lambda (key)
  			   ...
  			   (*:toString
    			    (invoke-special
    			     ...
    			     (this) 'get key))))
    			(clojure.core$keys:invokeStatic m))
    		   " ")))
       (string-append
        "(Bundle"
  ...
  (define-class HashBundle (clojure.lang.PersistentHashMap)
    ((*init* elements::(Object[]))
     (invoke-special clojure.lang.PersistentHashMap (this)
    		   '*init* elements))
    ((toString)::String
     (let ((content (string-join
  		   (map (lambda (key)
  ...
  (define (clojurize value)
    (cond
     ((keyword? value) ;<
      (clojure.lang.Keyword:intern (keyword->string value)))

     ((symbol? value) ;<
      (clojure.lang.Symbol:intern (symbol->string value)))
  ...
  (define (Bundle . args)
    (let ((n ::int (length args)))
      ;;(assert (even? n))
      (let ((args* (Object[] length: n))
    	  (i 0))
        (for-each (lambda (arg)
    		  (set! (args* i) (clojurize arg))
    		  (set! i (+ i 1)))
    		args)
        (if (<= n 16) ;>
    	  (VectorBundle args*)
  ...
  (define (datomic-local system ::String
     		       db-name ::String
     		       #!key (path '#:mem))
    ;;::Database
    (let* ((client-parameters (Bundle server-type: '#:datomic-local
     				    system: system
     				    storage-dir: path))
      	 (client (datomic.client.api$client:invokeStatic
     		  client-parameters))
    	 (connection-parameters (Bundle db-name: db-name)))
      (datomic.client.api$connect:invokeStatic
       client connection-parameters)))

  (define (database-snapshot database #;Database)
    ;;DatabaseSnapshot
    (datomic.client.api$db:invokeStatic database))

  (define the-database
    (make-parameter (datomic-local "system" "base")))

#+END_SRC

#+BEGIN_SRC java

    ~/kawa-datomic/zips $ javap clojure-1.12.0/clojure/lang/PersistentHashMap.class |head -n 2
    Compiled from "PersistentHashMap.java"
    public class clojure.lang.PersistentHashMap
        extends clojure.lang.APersistentMap
        implements clojure.lang.IEditableCollection,
    	       clojure.lang.IObj,
    	       clojure.lang.IMapIterable,
    	       clojure.lang.IKVReduce {...}
    ~/kawa-datomic/zips $ javap clojure-1.12.0/clojure/lang/PersistentArrayMap.class |head -n 2
    Compiled from "PersistentArrayMap.java"
    public class clojure.lang.PersistentArrayMap
          extends clojure.lang.APersistentMap
          implements clojure.lang.IObj,
    		 ...
    		 clojure.lang.IKVReduce,
    		 clojure.lang.IDrop {...}
    ~/kawa-datomic/zips $ javap clojure-1.12.0/clojure/lang/APersistentMap.class |head -n 2                                                       Compiled from "APersistentMap.java"
    public abstract class clojure.lang.APersistentMap
    ...
    ~/kawa-datomic/zips $ javap clojure-1.12.0/clojure/lang/IPersistentMap.class
  Compiled from "IPersistentMap.java"
  public interface clojure.lang.IPersistentMap
  	extends java.lang.Iterable,
  		clojure.lang.Associative,
  		clojure.lang.Counted {
    public abstract clojure.lang.IPersistentMap assoc(java.lang.Object, java.lang.Object);
    public abstract clojure.lang.IPersistentMap assocEx(java.lang.Object, java.lang.Object);
    public abstract clojure.lang.IPersistentMap without(java.lang.Object);
    public default clojure.lang.Associative assoc(java.lang.Object, java.lang.Object);
  }
...
No dobra, to na razie bedziemy robili kilka prob
z systemem Datomic.

Celem jest integracja Datomika z Kawa, co jednak bedzie
rowniez wymagalo integracji z Clojure, co z kolei daje
podstawe do narodzin projektu =Snaj=

I teraz jak sie to ma wydarzac:

chcielibysmy zasadniczo polaczyc sie z baza Datomic
z poziomu Kawy, ale zeby to zrobic, musimy najpierw
dobrze zrozumiec jak to wyglada z poziomu Clojure.

No to po pierwsze jest sobie cos takiego jak
=datomic.client.api/client=, ktorego argumentem
jest mapa posiadajaca przynajmniej takie klucze:
=:server-type=, =:system=.

No dobra, to jak bysmy chcieli tej bazy uzywac?

#+BEGIN_SRC scheme

  (parameterize ((the-database (datomic-local
  			      "movies"
  			      path: "/storage/emulated/0/datomic"
  			      )))
    (assuming `((,e1 a1: v1)
  	      (,e1 a2: v2))
  	    (select (variables ...)
...
A gdyby tak zrobic wersje GRASPa w ktorej wszystkie
struktury danych sa oparte o baze Datomic?

W ogole moze warto by bylo zrobic eksperyment
z uzywaniem Kawy do pracy z Datomikiem? Czy to
by bylo interesujace?

Czyli zasadniczo by to wygladalo w taki sposob, ze:
- kazdy byt odpowiada umiejscowionemu obiektowi

* 29-28.03.2025 (komputer)

Rzecz wyglada w taki sposob, ze z jakichs powodow
na razie nie chcemy babrac sie w tym kodzie, ktory mamy,
i chcielibysmy sobie przemyslec to i owo.

Po pierwsze, chcielibysmy zmienic reprezentacje kursora
na ArrayList. Podczas trawersowania drzewa bysmy po prostu
odkladali rzeczy na stos.

Czyli kontekst to bedzie w zasadzie mutowalny =Deque=
ktory bedzie oparty na =ArrayList=.

Po drugie, jezeli naciskamy jakis klawisz, to chcielibysmy,
zeby jego obsluga byla wywolywana kaskadowo, tzn. zeby
poszeczegolne funkcje byly propagowane zgodnie z kursorem.

Po trzecie, operacje musza byc w jakis sposob zreifikowane,
zeby dalo sie na nich przeprowadzac "undo".

* 28.03.2025

Wydaje sie, ze wielki redesign GRASPa zbliza sie
wielkimi krokami. Przy czym:

- chcielibysmy pewnie moc rowniez edytowac pliki
  Clojure i Common Lispa
- musimy przemyslec sobie reprezentacje dokumentu
- a moze chcielibysmy sobie tego uzywac jako Datomika?

* 27.03.2025

To my w koncu zrobimy to sterowanie z klawiatury?
Po ostatniej aktualizacji Androida warto by bylo tez
sie zastanowic nad optymalizacja tego nieszczesnego
renderowania

No ale to - jak to mowia - maniana

Teraz zrobmy tak (w handlerze klawiatury =ColumnGrid=), ze:
- jezeli =(is (the-cursor) proper-suffix? context)=,
  to robimy =(set! (the-cursor) (recons (first-index) context)=
- a jezeli =(equal? context (cdr (the-cursor)))=

* 26.03.2025

Ech, ach i och.

GRASP dziala niemozliwie wolno na Androidzie po przywroceniu
ustawien fabrycznych.

Stad przychodzi kilka nowych pomyslow.

Po pierwsze, byloby niezle gdyby stworzyc cos, co mozna
by bylo kompilowac do javascriptu, albo do WASMa.

Po drugie, byloby super, gdyby dalo sie to rowniez
kompilowac natywnie (moze byc do Chez/Guile/Chicken).

Po trzecie, byloby extra, gdyby opcja kompilacji do JVM
pozostala.

Moze tez warto rozwazyc opcje kompilacji do DartVM?

* 24.03.2025

No dobra, to teraz plan jest taki, ze chcemy zeby:
- strzalki w gore i w dol robily nam poruszanie
  po menu
- strzalki lewo i prawo oraz backspace, a takze
  wszystkie literki robily nam edycje pola tekstowego

I to jest jeden aspekt.
Drugi jest taki, ze mamy - przynajmniej na Androidzie
- mozliwosc cache'owania

https://stackoverflow.com/questions/52642055/view-getdrawingcache-is-deprecated-in-android-api-28

Jeszcze bysmy musieli obczaic, czy AWT tez daje takie
mozliwosci - albo zamiast AWT uzyc czego innego?

* 22.03.2025

dobrze by bylo chociaz przetestowac czy to
kolorowanie, ktore zrobilismy ostatnio,
w ogole jakkolwiek dziala.

No to rzecz by tak wygladala, ze lewo albo prawo
albo dowolna literka musialyby powodowac ustalenie
jako kursora kontekst + sciezke do pola tekstowego,

* 20.03.2025

Plan bylby taki, zeby:
- dodac ciemniejszy kolor dla zaznaczonych pozycji
  (dla guzikow i tych gridow nieszczesnych)
- zrobic pelna mozliwosc nawigowania z klawiatury
- i moze przy okazji dodac odwrocenie kolorow
  na 200ms przed wykonaniem akcji?

No to tak: mamy sobie metode =FileButton:draw!=,
i chcemy do niej dodac sprawdzenie kursora/
contextu

Jak teraz wyglada to w roznych przypadkach?

Na pewno mamy renderowanie pudelka:
...
No to tak: w nieodleglym czasie bysmy sprobowali
zrobic cos, zeby dalo sie poruszac po menu za
pomoca klawiarury - co wiaze sie z pewnymi
wyzwaniami zblizonymi do poruszania sie po
przyciskach

Ponadto celem nadrzednym powinno byc rozpoczecie
uzywania GRASPa do edytowania jego samego, a
w szczegolnosci zintegrowania skryptow do budowania
projektu.

Docelowo chcemy rowniez:
- przegladarke modulow
- algorytm A* i uporzadkowanie kwestii rozszerzen

* 18.03.2025

To bysmy sie zajeli tym, zeby mozna bylo
(przynajmniej w jakims zakresie) obslugiwac
GRASP z klawiatury.

* 16.03.2025

No to teraz sytuacja wyglada w taki sposob, ze
nie bardzo wiemy, co chcemy zrobic, ale uzywanie
edytora na telefonie jest z cala pewnoscia
problematyczne.

Może spróbujmy naprawić zachowanie kursora
tekstowego w okienku do zapisywania.

No dobra, to jest jakas mysl, aczkolwiek
docelowo chcielibysmy tez obsluzyc nawigacje
z klawiatury, oraz filtrowanie plikow po nazwie.

To bysmy chcieli stworzyc cos a'la KeyMap


* 15.03.2025

Jakies pomysly na dalszy rozwoj?

Najlepsze co przychodzi do glowy to:
- stworzyc dialekt, ktory sie kompiluje
  i do javy, i do javascriptu (i ewentualnie
  do C albo do Scheme'u)
- i w oparciu o to stworzyc nowego klienta,
  ktory by uzywal 'grzebieniowej' reprezentacji
  wyrazen

A co z aktualnym klientem?

Wydaje sie, ze dalej mamy miejsce na rozwoj.

I przede wszystkim nalezaloby zaczac od tego, zeby
sprobowac uzywac GRASPa do edytowania jego samego.

A co jesli nie?

Bo z takich rzeczy, ktore sa niejako ortogonalne,
to jest:
- GutenBergSon
- algotyrm A*
- przegladarka grafow
...
A poza tym:
- scroll-bary
- ogolne poprawki w edycji i 'doswiadczeniu'

Na razie mozemy sie zajac zamykaniem dokumentu,
bo mamy niezaimplementowana funkcje =close-document!=.

Zamykanie dokumentu powinno dzialac w taki sposob, ze:

* 07.03.2025

Czego my tak naprawde teraz chcemy?

- z jednej strony chcemy zakodowac algorytm A*
w komiksowej formie

- z drugiej strony chcemy, zeby nasz edytor byl
uzyteczny

- z trzeciej strony chcemy przerzucic sie z emacsa
na nasz edytor

- z czwartej strony chcemy miec przegladarke grafow
/projektow

Ale chcemy, czy nie chcemy tak naprawde?

Albo jeszcze kilka innych kwestii.
Po pierwsze, podgladanie stanu obiektow na zywo.
Po drugie moze jakies cache'owanie renderowania.

* 04.03.2025

Poniewaz nie chcemy przesadnie sie angazowac w robienie
engine'u fizycznego, byc moze warto by bylo rozwazyc
integracje z dyn4j.

Gdybysmy jedbak chcieli to robic, to z pewnoscia
powinno sie to odbywac na osobnym branchu.

Na razie byc moze warto by bylo raczej sie skupic
na pisaniu ksiazki, a w wolnych chwilach poprawiac
bugi i wszystkie inne problemy, jakie stwarza
nam GRASP, zeby mozna go bylo wreszcie wydac.

* 03.03.2025

Czyli co teraz?

Zabawa w dotykanie kulek?

A co z:
- przegladarka modulow?
- otwieraniem i zamykaniem pudelek?
- skomiksowanym algorytmem A*?
- wyszukiwaniem?

* 01.03.2025

to moze dzis sprobujmy zrobic to ciaganie palcem
po ekranie

no i w jaki sposob to musi dzialac?

po pierwsze, musimy zrobic te wszystkie funkcje
do obslugi myszy/dotyku, czyli:
- w momencie kiedy dotykamy widgetu, to przeszukujemy
  cala scene w poszukiwaniu elementu
- i teraz mozemy albo "transcendowac", tzn. wyjac
element, albo dzialac jakas sila

* 28.02.2025

spróbujmy może to rozwiązać, bo czasu jest mało

#+BEGIN_SRC
java.lang.NullPointerException
        at java.base/java.io.File.<init>(File.java:278)
        at editor.input.ParentDirectory.<init>(document-editor.scm:164)
        at editor.input.document$Mneditor.contentWithParent$St(document-editor.scm:216)
        at editor.input.document$Mneditor.openFileBrowser(document-editor.scm:267)
#+END_SRC

* 27.02.2025

teraz bysmy chcieli zrobic cos troche-jakby nowego,
czyli mozliwosc ciagania tych kulek za pomoca palca/myszy

troche tez chyba mamy odpowiedz na kolizje do poprawy,
bo dosc dziwne rzeczy sie dzieja

* 25.02.2025

mamy juz sobie sczytywanie akcekerometru na androidzie.
teraz trzeba to jeszcze wepchnac w jakis interfejs.

No to pomysl jest taki, zeby po prostu miec wektor
/parametr *the-gravity* i zwyczajnie przepisywac do
niego wartosci we funkcji.

* 24.02.2025

No to teraz plan jest taki, ze chcemy podpiac
akcelerometr do kulek

* 23.02.2025

No to probujemy rysowac wektor normalny przy kolizji,
a po drugie chcemy zrobic tak, ze jezeli mamy kolizje,
ale kierunki wektorow predkosci ida na przekor normalnym
kontaktu, to wtedy nie stosujemy sily

No to sprawdzmy moze, jakie sa cosinusy, tzn. iloczyn
skalarny normalnej kontaktu i normalnej predkosci

* 21.02.2025

Teraz chcemy sie skupic na diagnostyce problemow
z detekcja kolizji.

Zaczelibysmy moze od tego, zeby rysowac wektory
kontaktu w momencie kolizji

Ale zeby to moglo dzialac, potrzebujemy funkcji
=precise-draw-line x0 y0 x1 y1 color=
Ale w sumie to latwizna, bo w terminalu to
po prostu =8pix-draw-line!=, a w pozostalych
klientach uzyjemy funkcji dostarczanej przez
graficzne API

No, ale w sumie to my juz to mamy ;P


* 19.02.2025

No dobzie, to musimy sie przyjrzec, dlaczego
te kolizje nie dzialaja.

W tym celu bysmy stworzyli widget =PhysicsPlayer=,
za pomoca ktorego moglibysmy sterowac symulacja
fizyczna.

Ale najpierw musielibysmy przeniesc definicje
interfejsow do =elements=, a =PlayerWithControls=
do =widgets= (czy cos)

* 18.02.2025

To dzis sprobujemy przede wszystkim uruchomic nasz
symulatorek.

Aaale jeszcze trzeba dodac sciany do symulatora
fizyki.

No dobea, w teorii udalo sie symulator odpalic, tylko
jakos odbicia nie dzialaja, i w terminalu ta skala cos
jakos nie dziala.

Ale rodza sie z tego dlanas plany na kolejny tydzien:
- zeby podpiac akcelerometr, dotyk a takze orientacje
  dokumentu
...
chcemy dodac do =Painter= metode
=precise-fill-rectangle!=

Oke, wydaje sie, ze to juz mamy. I teraz nam pozostaje:
- implementacja odpowiedzi na kolizje
- przetestowanie tego, czyli zbudowanie
  sceny "fizycznej" z jakims obiektem
- podpiecie akcelerometru!

To sobie przypominamy wzorek:

~dP = (1+C)m1*m2*(v2-v1)/(m1 + m2)~

gdzie =(v2-v1)= to roznica rzutow predkosci
na os zderzenia

~1/dP = (m1 + m2)/((1+C)*m1*m2*(v2-v1))~

~1/dP = (m1 + m2)/((1+C)*m1*m2*(v2-v1))~

~1/dP = 1/((1+C)*m2*(v2-v1)) + 1/((1+C)*m1*(v2-v1))~

~1/dP = (1/m1 + 1/m2)*(1/((1+C)*(v2-v1)))~

~((1+C)*(v2-v1))/dP = (1/m1 + 1/m2)~

~dP = (1+C)*(v2-v1)/(1/m1 + 1/m2)~



* 16.02.2025

mamy juz kolizje pomiedzy kula a sciana.

teraz pozostaje nam rysowanie scian i odpowiedz
na kolizje

* 14.02.2025

Dzisiaj chcemy zrobic:
- uproszczona kolizje pomiedzy
  kula a sciana
- rysowanie scian
- odpowiedz na kolizje

(tak naprawde jezeli zrobimy tylko
detekcje kolizji, to bedzie niezle)

To zacznijmy od tego:
- odleglosc punktu =p0= od odcinka =|AB|=

  =d((x0,y0),|(xA,yA)(xB,yB)|)=

  przy zalozeniu, ze ~xA=xB~ lub ~yA=yB~

  - jezeli ~xA=xB~, to odleglosc od prostej,
    na ktorej lezy odcinek, wynosi =|x0-xA|=.

    i teraz jezeli ~min(yA,yB) <= y0 <= max(yA,yB)~,
    to wlasnie to bedzie nasza odleglosc.

    jezeli jednak ten warunek nie zachodzi,
    to bierzemy =hypot(|x0-xA|, min(|y0-yA|,|y0-yB|))=

  - analogicznie dla przypadku ~yA=yB~


* 13.02.2025

Chcemy przemianowac stworzona wczoraj =AnimateCanvas=
na =PhysicalStage=, dodac =ContactPoint=, i rozszerzyc
aktualizacje o detekcje kolizji.

To do uruchomienia symulatora potrzebujemy jeszcze:
- rysowania prostokatow
- detekcji kolizji
  - sfera sfera
  - sfera pudelko
  - pudelko pudelko
- obslugi odpowiedzi na kolizje

* 12.02.2025

No to dzisiaj bysmy moze zrobili tak, ze bysmy
przywrocili jako =Collider= =AxisAlignedBoundingBox=,
i zdefiniowali sobie jako krawedzie:

#+BEGIN_SRC scheme
  ;; lewa sciana
  (AxisAlignedBoundingBox
   left: -inf.0 right: 0
   top: -inf.0 bottom: +inf.0)

  ;; sufit
  (AxisAlignedBoundingBox
   left: -inf.0 right: +inf.0
   top: -inf.0 bottom: 0)

  ;; podloga
  (AxisAlignedBoundingBox
   left: -inf.0 right: +inf.0
   top: height bottom: +inf.0)
  
  ;; prawa sciana
  (AxisAlignedBoundingBox
   left: width right: +inf.0
   top: -inf.0 bottom: +inf.0)

#+END_SRC

Musimy sie jeszcze upewnic, ze =PreciseCanvas= w =TextPainter=
obsluguje wspolrzedne pseudorzeczywiste.

Wyglada na to, ze powinno.

Czyli jaki jest kolejny krok?

Na pewno musimy:
- przywrocic =AxisAlignedBoundingBox=
- zdefiniowac sciane
- zdefiniowac kule
- odbicia pomiedzy kulami i scianami, kulami i kulami

No dobra, mamy bounding-boxy.

I co dalej?

* 11.02.2025

Musimy jeszcze poprawic formule wyliczajaca impuls

#+BEGIN_SRC scheme
  (define-type (PhysicalSphere mass: real
			       velocity: (sequence-of
					  real)
    ...
    ...
    ((collide-with! another ::Body)::void
     (let ((points (invoke-special Colliding (this) 
				   'contact-points
				   another)))
       (for contact::ContactPoint in points
	  (let* ((m1 mass)
		 (m2 (another:current-mass))
		 (v1 (apply + (map * contact:direction
				   velocity)))
		 (v2 (apply
		      + (map * contact:direction
			     (another:current-velocity))))
		 (C (* bounciness
		       (another:current-bounciness)))
		 (impulse (/ (* (+ 1 C) m1 m2 (- v2 v1))
			     (+ m1 m2))))
	     (impulse! impulse contact)
...
j = (-1-C)Vrel0
    /(  1/mA
      + 1/mB
      + nB*(IAinv (rA x nB)) x rA
      + nB*(IBinv (rB x nB)) x rB )

#+END_SRC

Ale to dziala w ogolbym przypadku, gdy wyrazamy moment
pedu jako wektor.

Zastanowmy sie przez chwile nad sensem fizycznym
poszczegolnych skladowych. rA x nB to wektor
prostopadly do plaszczyzny sily/srodka masy.
Jezeli IA jest diagonalna, to spowoduje tylko
zmiane dlugosci tego wektora. Dalej: iloczyn tego
wektora przez wektor rA da w wyniku wektor lezacy
mniej wiecej w plaszczyznie wektora nB, natomiast
iloczyn skalarny da nam rzut otrzymanego wektora.

W podstawowce na fizyce mielismy takie cos, ze
dlugosc iloczynu wektorowego to iloczyn dlugosci
razy sinus kata miedzy nimi. Wiemy, ze nB jest
jednostkowy, i wiemy, ze (rA x nB) jest prostopadly
do rA (oraz do nB). Zakladajac, ze IA to jednorodna
macierz diagonalna, wiemy, ze IAinv (rA x nB) x rA
ma dlugosc rA^2*sin(<rA,nB)/m , i ze to samo
mozemy mutatis mutandis powiedziec o rB.

Rzecz jest jednak taka, ze:
- w pierwszym rzucie mozemy to olac, i skupic sie
  na kulach
- docelowo modelowalibysmy nawet ciala 2d poslugujac
  sie wektorami 3d, bo dla takich jest zdefiniowany
  iloczyn wektorowy

No dobra, czyli dzisiaj zajmujemy sie integracja
tego, co juz mamy, zeby pozyskac przynajmniej
dzialajacy symulator z odbijajacymi sie kulami.

I jeszcze bysmy dodali polplaszczyzne jako =Collider=.

No wlasnie: bo tutaj rzecz ma sie tak, ze gdybysmy
wzieli nasza formule i chcieli ja zastosowac
do czegos o nieskonczonej masie, to by nam wyszlo
mnozenie i dzielenie przez ~+oo~
...
#+BEGIN_SRC
  ...
  1/dP = (m1+m2)/((1+C)*m1*m2*(v2-v1))
  = 1/((1+C)*m2*(v2-v1)) + 1/((1+C)*m1*(v2-v1))
  
#+END_SRC

I jak to sobie w taki sposob zapiszemy, to dostajemy
dwa skladniki zalezne od poszczegolnych mas
- i jezeli jedna z tych mas bedzie nieskonczona,
  to skladnik po prostu sie wyzeruje

A wezmy jeszcze tego Czecha, ale w uproszczeniu,
#+BEGIN_SRC
  (-1-C)(v1-v2) = j(1/m1 + 1/m2)

  1/j = (1/m1 + 1/m2)/((1+C)*(v2-v1))
#+END_SRC

No to sie to nadal zgadza. Pozostale skladniki
mozemy rowniez wyliczyc przed odwroceniem.

No to co? =inverted-momentum-component=

Moze na razie dodajmy do =PreciseCanvas= =BiMap2D=
i zawinmy to w =Bordered=?

* 10.02.2025

#+BEGIN_SRC scheme

  (define-interface Body (Alive Colliding)
    (impulse! force ::real
    ...
    (current-inertia)::real
    (current-angular-velocity)::real
    (current-bounciness)::real
    )

  (define (cross-product-2d a::(sequence-of real)
			 b::(sequence-of real))
    (- (* (a 0) (b 1))
       (* (a 1) (b 0))))
  ...
  (define-type (PhysicalSphere mass: real
    ...
    ((current-velocity)::(sequence-of real) velocity)
    ((current-inertia)::real inertia)
    ((current-angular-velocity)::real angular-velocity)
    ((current-bounciness)::real bounciness)

    ((impulse! value ::real contact-point ::ContactPoint)::void
     (for i::int from 0 below (length velocity)
       (set! (velocity i)
	 (+ (velocity i) (* value
			    (contact-point:direction i)
			    (/ mass)))))
     (let* ((arm (map - contact-point:position 
			(slot-ref (this) 'center))))
       (set! angular-velocity
	 (+ angular-velocity
	    (* value (cross-product-2d
...
Co chcemy zrobic:
- uwzglednic zmiane momentu pedu przy oddzialywaniu
  popedu
- wyliczac poped podczas zderzenia
...
#+BEGIN_SRC scheme
  ...
  (define-interface Body (Alive Colliding)
    (impulse! force ::real contact-point ::ContactPoint)::void
    (collide-with! another ::Body)::void
    (mass)::real
  ...
  (define-type (PhysicalSphere mass: real
    ...
    ((impulse! value ::real contact-point ::ContactPoint)::void
     (for i::int from 0 below (length velocity)
       (set! (velocity i)
	 (+ (velocity i) (* value (contact-point:direction i) (/ mass)))))
     (let* ((arm (map - contact-point:position 
			(slot-ref (this) 'center))))
       ...))
    ...
    ((collide-with! another ::Body)::void
     (let ((points (invoke-special Colliding (this) 
				   'contact-points another)))
       (for contact::ContactPoint in points
	  (let ((impulse ...))
	     (impulse! impulse contact)
	     (another:impulse! (- impulse) contact)))))

    implementing Alive with
    ((advance! timestep/ms::int)::boolean
     (for item ::Body in content
  ...
  (define-object (PhysicalWorld content::(list-of Body))
    ::World

    (define (advance! timestep/ms::int)
	::boolean
	(for item ::Body in content
	  (item:advance! timestep/ms))
	(for (a ::Body b ::Body) in (collisions content)
...
Wydaje sie, ze wazne jest to, zeby dokonac
rozdzielenia fizyki od geometrii.

To znaczy, =Collider= jest czyms geometrycznym,
natomiast =Body= jest czyms fizycznym.

Wiec do wyboru mamy albo delegowanie, albo
dziedziczenie. Wydaje sie jednak, ze skoro
rzeczy fizyczne opieraja sie na tych
geometrycznych (przede wszystkim polozenie),
lepiej jest wspoldzielic ich reprezentacje
poprzez dziedziczenie wlasnie.

OK, mamy na razie punkty kontaktowe. Teraz bysmy
chcieli przylozyc impuls do tego punktu.


* 06.02.2025 (poranek)

No dobra, czyli mamy cos w stylu
...
#+BEGIN_SRC
  ...
  Wa' = Wa + Iainv(rA x jnB)

#+END_SRC

Czyli teraz wiemy, ze gdy wyznaczamy
punkt kontaktu, to oprocz samego punktu
potrzebujemy rowniez wektora normalnego.

Pytanie brzmi: co dalej?

W miare mozliwosci bysmy chcieli opracowac
interfejs generyczny, czyli zeby ten sam kod
dzialal w 2D i w 3D.

OK, to w teorii bysmy mogli sobie juz teraz
wygeneralizowac =Box2D= na =HyperCube=, a
=Circle= na =HyperSphere=, i w pierwszym przypadku
reprezentowac calosc jako tablice wartosci
=(poczatek, koniec)=, a w tym drugim reprezentowac
srodek jako tablice wspolrzednych.

Na razie jednak to nie jest naszym celem. Wazniejsze
wydaje sie stworzenie klasy =Polygon=, ktora jednak
trudniej bedzie uogolnic (w 3d bylby to =Mesh=).

OK, to teraz jeszcze kwestia momentow bezwladnosci.
Dla kuli glowne momenty to =2/5mr^2=, dla sfery
=2/3mr^2=, dla walca moment dla osi, w ktorej przekroj
jest kolisty, to =1/2mr^2= (we wszystkich przypadkach
momenty dewiacyjne sa 0), dla srodka preta =1/12ml^2=,
dla krawedzi preta =1/3ml^2=, dla prostokata to
=1/12m(w^2+h^2)=.

W kazdym razie tensor momentu bezwladnosci sie nam
redukuje do pojedynczej liczby.

#+BEGIN_SRC scheme

  (define-interface Body (Alive)
    (shape)::Collider
    )

  (define-type (ContactPoint position: Position
			     direction: Position
			     impulse: real))

  (define-object (PhysicalBody2D ...)::Body
    (define horizontal-velocity ::real 0.0)
    (define vertical-velocity ::real 0.0)
    (define angular-velocity ::real 0.0)
    (define left ::real 0.0)
    (define top ::real 0.0)
    (define rotation ::real 0.0)
    (define mass ::real 1.0)
    (define inertia ::real 1.0)

    (define (impulse! momentum ::real
		      position ::Position
		      direction ::Position)
      ::void
      ;; czysta fizyka
    ...
    (define (contact-points another::PhysicalBody2D)
      ::(list-of ContactPoint)

      ...)

    (define (collide-with! another::PhysicalBody2D)
      ::void
      (let ((contacts (contact-points another)))
	(for contact in contacts
	  (let ((impulse (contact:impulse a b)))
	    (a:impulse! contact:impulse
			contact:position
			contact:direction)
	    (b:impulse! (- impulse) contact:position
			contact:direction)))))

    (define (advance! timestep/ms::int)
      ::boolean
      (set! left
	(+ left (* horizontal-velocity
		   timestep/ms)))
      (set! top
  ...
  (define-type (PhysicalWorld2D
		content::(list-of
			  PhysicalBody2D))
    (define (advance! timestep/ms::int)
      ::boolean
      (for item::PhysicalBody2D in content
	(item:advance! timestep/ms))
      (for (a::PhysicalBody2D
	    b::PhysicalBody2D)
	  in (collisions #;between content)
	(a:collide-with! b)))
    ...)

#+END_SRC

No dobra, jeszcze dochodzi nam taka rzecz, ze jezeli
chcemy wyabstrahowac dynamike od samego rysowania
/rozciaglosci, to musimy byc w stanie wyliczyc =Extent=
dla dowolnego kata, ktory musi przychodzic z zewnatrz.

Ok, ale co chcemy zrobic dalej?
- przede wszystkim, wyznaczanie punktow kolizji
  dla kolek, a w dalszej kolejnosci dla wielokatow
- usuniecie =Box2D=, generalizacja =Circle= do =BoundingSphere=
  (rowniez zeby uniknac kolizji z =Circle= w =PreciseCanvas=)

Na pewno bedziemy chcieli:
- zaimplementowac =Polygon= jako =Collider=
- ale wczesniej chcemy sie zajac kulami
- i do =BoundingSphere= bedziemy chcieli dodac =ContactPoints=


* 05.02.2025 (wieczor)

Wyglada na to, ze sie udalo zrozumiec jeden
z pejperow - gdzie wyznaczona zostaje plaszczyzna
kolizji, a nastepnie jest w jej ramach rozwiazany
...
i predkosc zostaje zaktualizowana wlasnie w owym
rzucie.

Oczywiscie, w gre wchodzi tutaj jeszcze taka
kwestia, ze rownanie zostaje rozwiazane dla
odbicia "idealnie sprezystego", tj. przy zalozeniu
zachowania energii kinetycznej.

Natomiast w praktyce chcielibysmy na pewno,
zeby istniala mozliwosc "pochlaniania" energii
przy uderzeniu (na przyklad, zeby "zniszczyc"
zbroje), i z pewnoscia chcielibysmy parametryzowac
uderzenia procentem pochlonietej energii.

Druga rzecz jest taka, ze w pewnych sytuacjach
bedziemy mieli do czynienia ze "zlamaniem"
zasady zachowania pedu (np. rzeczy przytwierdzone
do ziemi nie beda jej podlegac), dlatego nie ma
sie co przesadnie nad tym spuszczac.

Wrocmy jednak do naszych rownan:
...
Chcemy wyznaczyc sile, czy moze raczej
"poped", ktory dziala na obydwa ciala.

Co wiemy?

~F = m*a = dp/dt~

~Impuls = Calka(t0, t1) F(t)dt~

W naszym przypadku zakladamy, ze krok symulacji
bedzie staly, czyli mamy sobie jakies ~Δt~,
i zmiana pedu wyniesie ~Δp = F*Δt~.

I teraz tak: zmiana pedu pierwszego ciala wyniesie
~m1*(v1' - v1)~, a zmiana pedu drugiego ciala
~m2*(v2' - v2)~, choc to oczywiscie zalezy jak
na to patrzec.

Wiemy tez, ze ta liczba powinna byc w obu przypadkach
taka sama!

#+BEGIN_SRC
 m1*(v1' - v1) = m1*((2*m2*v2 + (m1 - m2)*v1)/(m1+m2) - v1)
 = m1*((2*m2*v2 + (m1 - m2)*v1)/(m1+m2) - v1(m1+m2)/(m1+m2))
 = m1*((2*m2*v2 + m1*v1 - m2*v1 - m1*v1 - m2*v1))/(m1+m2)
 = 2*m1*m2*(v2-v1)/(m1+m2) [kg*m/s]


m2*(v2'-v2) = 2*m1*m2*(v1-v2)/(m1+m2)
#+END_SRC

Sie to zgadza! (Z dokladnoscia do znaku)
[czym pewnie nie ma sie co podniecac, skoro wynika
z zalozen]

No dobra, to teraz ilekroc ktos nas zapyta o to,
jaki impuls dziala na dwa ciala w momencie uderzenia,
to mu powiemy, ze ~Δp = +-2*m1*m2*(v1-v2)/(m1+m2)~

Ale jaka to ma interpretacje?

* 05.02.2025 (poranek)

Wydaje sie, ze blad we wczorajszym wyprowadzeniu
jest taki, ze o ile zasade zachowania pedu mozna
rzutowac na poszczegolne osie (a mozna?), o tyle
w przypadku energii juz nie jest tak prosto, bo
do kwadratu podnosimy wielkosci wektorowe,
czyli v^2 to kwadrat z dlugosci wektora, co mozna
zapisac jako vx^2 + vy^2, czyli zasade zachowania
energii w 2d mozna zapisac jako

#+BEGIN_SRC
  m1*(v1x^2 + v1y^2) + m2*(v2x^2 + v2y^2)
  = m1*(v1'x^2 + v1'y^2) + m2*(v2'x^2 + v2'y^2)
#+END_SRC

natomiast zasade zachowania pedu mozemy rozpisac
jako
...
Szukamy v1'x, v1'y, v2'x, v2'y
ale mamy 4 niewiadome, a tylko 3 rownania.
Niech Px = m1*v1x + m2*v2x,
Py = m1*v1y + m2*v2y, E1 = m1*(v1x^2 + v1y^2),
E2 = m2*(v2x^2 + v2y^2), E=E1+E2wszystko to sa
wielkosci dane

#+BEGIN_SRC
Px = m1*v1'x + m2*v2'x
Py = m1*v1'y + m2*v2'y
E= m1*(v1'x^2 + v1'y^2) + m2*(v2'x^2 + v2'y^2)
#+END_SRC

Mozemy wyliczyc i podstawic
#+BEGIN_SRC
v1'x = (Px - m2*v2'x)/m1
v1'y = (Py - m2*v2'y)/m1
E=m1*(((Px - m2*v2'x)/m1)^2
     +((Py - m2*v2'y)/m1)^2)
  + m2*(v2'x^2 + v2'y^2)
#+END_SRC

No ale widac, ze to nie pomoze nam wyznaczyc
konkretnych wartosci v2'x i v2'y, a co najwyzej
ich wzajemna relacje.

I nic dziwnego - bo do wyznaczenia konkretnych
wartosci musimy jeszcze znac plaszczyzne odbicia!

No i fajnie. Czyli nadal g*wno wiemy.

Ale wezmy to video tego goscia, co mowi
o symulacjach fizycznych, i zerknijmy moze
do materialow, o ktorych opowiada.

* 04.02.2025

Symulator fizyczny: nowe rozdanie. To cosmy wczoraj
nawymyslali pozostaje w mocy.

Ale musimy sie jeszcze skupic na zamodelowaniu
zderzen
...
v1+v1'=v2'+v2
v1'=v2'+v2-v1
2*v1-v2'-v2=(m2/m1)(v2'-v2)
2v1 - v2' - v2 = v2'*m2/m1 - v2*m2/m1
v2'(m2/m1 - 1) = v2(m2/m1 - 1) - 2v1

cos tutaj jest nietenteges, a gostek wkleja tam jakies
magiczne wartosci z dupy
...
No, to sytuacja nie wyglada jak na razie
najlepiej. Rzecz w tym, ze chcemy wyznaczyc
sily dzialajace przy zderzezniach.

Zakladajac, ze podczas zderzenia energia
nie jest wytracana (np. przez sprezystosc
materialu), to w przypadku liniowym
zderzenia dwoch kul mamy z zasady
zachowania pedu
...
No dobra, to teraz jeszcze sie wciaz chcemy skupiac
na tym naszym symulatorze (i zadowoleni bedziemy
dopiero jak to podepniemy do akcelerometru)

No dobra, to na razie mamy taka ontologie, ze na
=PreciseCanvas= moze sie znalezc cos co jest =Renderable=.

Ale to rozwiazanie ma powazna wade, mianowicie nie
wyodrebnia "punktu mocowania" od rysowania. Bo my
bysmy chcieli albo rysowac bezkontekstowo, albo
w kontekscie - przy czym za umiejscawianie rzeczy
w kontekscie powinno byc odpowiedzialne *cos innego*.

No dobra, to jeden aspekt - i to w sumie dosc istotny.

Do tego dochodzi kwestia samych praw fizyki
(i ewentualnego przejscia z 2d do 3d).

No bo musimy miec metode =advance!=, ktora calkuje nam
ruch poszczegolnych obiektow, a w przypadku samej sceny
odpala detekcje kolizji i stosuje wszystkie niezbedne
sily.

No dobra, to teraz musimy sie zdobyc na takie bardziej
calosciowe spojrzenie:

Mamy modul =(extra collisions)=, ktory definiuje algorytm
detekcji kolizji, ale poza tym definiuje interfejs =Collider=
wymagany do dzialania algorytmu.

I teraz co?

Na pewno nasza klasa =PhysicsStage= bedzie musiala miec
liste takich =Collider=-ow

#+BEGIN_SRC scheme
  (define-interface Body (Alive Collider))
  ...
  (define-type (PhysicalWorld2D
	...
	(item:advance! timestep/ms))
      (for (a::PhysicalBody2D
	    b::PhysicalBody2D)
	  in (collisions #;between content)
	(let ((points (a:contact-points b)))
	  (for contact in contacts
	    ;; No, i tutaj rzecz tak
	    ;; wyglada, ze jak mamy
	    ;; zderzenie, to sila tego
	    ;; zderzenia powinna zalezec
	    ;; od mas i predkosci
	    ;; poszczegolnych obiektow,
	    ;; A co to oznacza w praktyce?
	    ;; Po pierwsze, wiemy, ze
	    ;; w momencie zderzenia albo
	    ;; jest tak, ze calkowita
	    ;; energa (kinetyczna) jest
	    ;; zachowana. Ale to jest
	    ;; prawda tylko dla obiektow
	    ;; o skonczonej masie, natomiaast
	    ;; obiekty nieruchome chcielibysmy
	    ;; modelowac jako obiekty o
	    ;; masach nieskonczonych (o ile
	    ;; to ma sens?)
	    ;; W kazdym razie wiemy tyle,
	    ;; ze powinny obowiazywac
	    ;; zasady zachowania energii
	    ;; i pedu.

	    ;; No ale co to nam mowi o
	    ;; silach, jakie powinny
	    ;; dzialac na uklad?

	    ;; Jezeli mamy kulke, ktora
	    ;; spada na podloge, to jezeli
	    ;; ta kulka jest idealnie
	    ;; sprezysta, to sie odbije
	    ;; od tej podlogi. Oznacza
	    ;; to, ze ped zmieni swoj zwrot
	    ;; na przeciwny.

	    ;; W przypadku kuli bilardowej
	    ;; odbijajacej sie od scianki
	    ;; stolu bilardowego
...
No to teraz plan jest jaki?

Chyba bysmy chcieli wyodrebnic =Splittable= z =Embeddable=,
i tak zmodyfikowac detektor gestow, zeby sprawdzal, czy 
=(Splittable? (screen:content))=.

A jak juz to bedziemy mieli, to sie zajmiemy na powrot
symulatorem?


* 31.01.2025

OK, to dzis zacznijmy dzien od poprawek w podswietlaniu
ramki, a w dalszej kolejnosci bysmy zrobili tak, zeby
=Bordered= byl =Maximizable=.

No dobrze. Udalo sie zrobic zeby byl =Resizable=,
ale wartosc robienia z niego =Maximizable= przy
obecnej ontologii wydaje sie znikoma.

(problem jest taki, ze w =Embeddable= pojawiaja
sie rzeczy, ktore tak naprawde powinny byc =Splittable=,
ale nie ma w naszym slowniku takiego pojecia)

Teraz czym bysmy sie w dalszym ciagu zajeli?

Na pewno chcemy stworzyc widget do symulacji.
Czy w zasadzie kwestia bylaby taka, zeby po prostu
stworzyc jakikolwiek widget, ktoremu mozna zmieniac
rozmiary.

A tak w gruncie rzeczy to co chcielibysmy miec?
- symulator cial fizycznych, do ksiazki,
  i zeby go zintegrowac z tymi czarodziejskimi
  mechanizmami do detekcji kolizji
- moze tez chcielibysmy stworzyc projekt "wyrazania
  emocji" oparty na twarzach

  W ogole musimy znalezc jakies tematy do ksiazki,
  zeby sie nie zakopac, i moze tez zeby bylo im
  po drodze z naszymi zainteresowaniami.

Z drugiej strony jest tez kwestia samego edytora
i naprawiania w  nim bugow, ale to szczerze mowiac
zrobimy tak jak by sie nam chcialo.

  

* 30.01.2025

No dobra, moze bysmy sie zajeli tym, zeby jakos
ladniej podswietlac ramke =Bordered=?

Pomysl bylby z grubsza taki:
- jezeli kontekst ma postac =(#\[ . ,,cursor)=,
  to kolorujemy na kolor podswietlonego nawiasu
  lewa i gorna krawedz
- jezeli ma postac  =(#\] . ,,cursor)=, to
  prawa dolna krawedz
- jezeli kontekst jest sufiksem kursora, to
  rysujemy takie lekko szare

No, ale do tego celu musimy zmodyfikowac metode
=draw-border!=. Mozliwosci sa takie, ze albo
dodamy parametr, albo rozbijemy to na rysowanie
czterech linii, i to moze w dwoch funkcjach:
- =draw-horizontal-border-line!=
- =draw-vertical-border-line!=

Na pewno jednak 'leniwiej' byloby zaczac od parametru.
Moglibysmy przyjac, ze parametr moze miec cztery wartosci
#\[, #\], #\null albo dowolny inny znak (przyjmijmy, ze #\t)

#+BEGIN_SRC scheme
(subtype-of gnu.text.Char (either #\[ #\] #\t #\null))
#+END_SRC

OK, to na razie mamy to na poziomie szkieletu. Teraz bysmy
zrobili w klientach graficznych kolory. Zaczniemy od
klienta androidowego

* 29.01.2025

To dzien zaczynamy od:
- poprawki =cursor-under*= w =Bordered=
- zrobienie, zeby =Bordered= bylo =Maximizable=
- dodanie =min-width= i =min-height= (i ewentualnie
  =max-width= i =max-height=) do =Resizable=?

* 28.01.2025

No dobra, to teraz bysmy sprobowali stworzyc kombinator
=Resizing=, ktory by przyjmowal jako argument widget
=Resizable= badz =Maximizable=, i zmienial jego rozmiar;
moze na bazie kombinatora =Bordered=, a moze po prostu
bysmy rozszerzyli jego funkcjonalnosci?

No dobra, zalozmy ze tak. Ale co wowczas z =Maximizable=?

Ono jest zdefiniowane tak:

#+BEGIN_SRC scheme
(define-interface Maximizable (Embeddable ResizableEnchanted))
#+END_SRC

Moze jeszcze by do tego dolozyc predykat =can-be-maximized?=
A do =Resizable= - =can-be-resized?=

I wowczas bysmy mogli zrobic tak, zeby =Bordered= bylo
=Resizable= i =Maximizable=, ale moglo byc =resized/maximized=
tylko wtedy, gdy jego =element= jest odpowiednio =Resizable=
i =Maximizable= (i odpowiednio moglo byc =resized/maximized=).

No dobra, to sprawdzmy, czy te zmiany, ktore teraz mamy,
kompiluja sie i z grubsza dzialaja, i jezeli tak, to wprowadzmy
te kolejne (a jezeli nie, to naprawmy to co mamy teraz, zeby
mozna to bylo jutro scommitowac)

OK, wydaje sie, ze dziala. To teraz sprawdzmy, jakie
widgety sa =Resizable= i =Maximizable=.

Na pewno =Screen=, =cons=, =EmptyListProxy=,
=Stretched=,  =MaximizableWidget= i jego potomstwo
(czyli najpewniej tylko =LetterTileBoard=).

No dobra. To zrobmy to!

OK, i zrobilismy. I przy okazji udalo sie odkryc,
ze w metodzie =cursor-under*= klasy =Bordered=
jest chyba blad (ale to sie napraaaawi, oczywiscie).

Czyli teraz chcemy zrobic tak, zeby =Bordered=
bylo =Maximizable=. Tak. A co dalej?

Jeszcze bysmy do =Resizable= dodali metody
=min-width= oraz =min-height=.

* 27.01.2025 (telefon)

No to dzisiaj probujemy zrobic tak, zeby =EmptyListProxy=
byl =Resizable=.

* 26.01.2025 (komputer)

No dobra, to na razie udalo sie nam przeprowadzic
mala refaktoryzacje. Teraz kolejne etapy bylyby takie, zeby:
- dodac mozliwosc zmiany rozmiaru pustej listy
- stworzyc kombinator do zmiany rozmiaru zawartego wen elementu

* 24.01.2025 (telefon)
To co bysmy zrobili?

#+BEGIN_SRC scheme
  (define-object (PhysicsStage content::(list-of Physical))
    ::World

    (define (advance! delta/ms::int)::void
      (for item::Physical in content
	(item:advance! delta/ms))
      (for (a b) in (collisions #;among content)
	(a:force! ...)
	(b:force! ...)))
...
Ok. No ale fajnie by bylo miec takie cos, zeby moc
zmieniac rozmiar widgetu

* 24.01.2025 (komputer)

No dobra, to bysmy chcieli miec cos takiego jak
=Resizing=, co stanowiloby owijke dla czegos, co jest
=Maximizable=, a z drugiej co samo byloby =Maximizable=
i wspolpracowalo z undo

Ale zeby cosik na ten temat wnioskowac, dobrze byloby
sobie najpierw przypomniec co to wszystko znaczy.
...
No, to bysmy zmienili =Resize= tak, zeby zamiast =cons=
przyjmowala =Resizable=, i dostarczyli instancje
=set-size!= do =cons= oraz do =EmptyListProxy=.

* 23.01.2025 (komputer)

Warto by bylo sie zastanowic nad nastepujacymi tematami:
- ksiazka - co chcemy w niej napisac,
  ale jak na razie wiele wskazuje na to, ze chcemy:
  - zrobic widget z fizyczna symulacja
  - zrobic widget z "the youngest of Tom's brothers"
    (wykorzystujacy stepper)
  - uzasadnic skladnie jezyka Scheme
  - napisac sekcje o definiowaniu
  - napisac sekcje o meta-jezyku

A jaki dalej uklad rozdzialow?

Powiedzmy, ze cos takiego:
- pojecie reprezentacji
- fizyka: zasada zachowania pedu
- tarcie i wytracanie energii

- chemia: czasteczki wody; dlaczego woda jest mokra
- biologia - podstawowa wiedza o komorce, moze cykl
Krebsa, moze jakis wirus,
- moze tez symulacje populacji?

- logika - semantyka swiatow mozliwych

* 18.01.2025 (komputer)

To taki watek troche niezalezny, ale interesujace jest
to, w jaki sposob zintegrowac Kawe (i GRASPa) z baza Datomic.

#+BEGIN_SRC scheme

(define-parameter (current-database) ...)

(define-syntax select
  (syntax-rules (from where)
    ((select variables ... from database where conditions ...)
     ...)
    ((select 


(select entity where (entity _ _))


(insert (Bundle db/id: id movie/genre: "future governor") 
 where (id movie/title: "Commando"))

(describe (current-database))

#+END_SRC

No dobra, a nasze odpalanie bazy tak wyglada:
bin/transactor config/samples/mysql-transactor.properties

* 19.01.2025

Plan jest zatem na razie taki, zeby:
- zajac sie pracami nad widgetem z kulkami
- skupic sie na ksiazce

* 18.01.2025

Nie wiem czy w GRASPie chcemy cos robic. Moze warto by bylo
rozwazyc fork ktory by umial gadac z datomic?

Czy moze chcemy zrobic jakis widget, np. orbitujace kolko?

Ale wowczas byloby dobrze zrobic te zmiany z =PreciseCanvas=,
o ktorych pisalismy wczesniej.

* 17.01.2025

No i fajnie, i mamy sobie jakies mechanizmy do zarzadzania
animacjami, i teraz fajnie by bylo napisac cos, co by jakos
tego systemu uzywalo - a do tego najlepiej zeby jeszcze
robilo detekcje kolizji, oraz po prostu symulowalo srodowisko
fizyczne

Czyli bysmy zaczeli od tego, zeby stworzyc =AnimatedExtension=,
albo po prostu =Alive=.

* 16.01.2025

To teraz bysmy chcieli stworzyc =AnimatedExtension=,
tzn. zeby narysowanie tego sprawialo, ze do listy animacji
zostaje dodany biezacy obiekt.

Jeden pomysl na rozwiazanie tego bylby taki, zeby miec
dwa zbiory: =previously-visible?= i =currently-visible?=,
i zeby w trakcie renderowania dodawac do tych zbiorow
aktualnie rysowane obiekty.

No dobra, cos tam sie udalo wymodzic w sumie. Tylko zamiast
metody =stop!= w =AnimatedTile= powinnismy miec =stop-playing!=
w =Painter=.

* 15.01.2025


Rewizja systemu amimacji
========================

W module =(editor interfaces painting)=, w interfejsie
=Painter=, jest sobie taka oto metoda:

=(play! animation::Animation)::void=

gdzie interfejs =Animation= jest zdefiniowany tak:
...
Natomiast poszczegolne klienty implementuja sobie ten interfejs
roznorako, np. desktop robi tak:
...
gdzie =queue= to kolejka, na ktora przychodza wszystkie zdarzenia,
zas watek edycji pobiera te zdarzenia, i w przypadku obiektow
=Pending= (oraz innych procedur) po prostu je "wykonuje".

Z kolei na Androidzie jest jeszcze inaczej:
...
no dobra, to moze bysmy sprobowali sie zajac
poprawka w refleksji?

czyli by to wygladalo jakos tak
...
Super. Cos tam nam nawet dziala. Ale co chcemy robic
dalej?

No ogolnie fajnie by bylo, gdyby te kulki sie zderzaly,
i gdyby wokol canvasu byla jakas obwodka, i gdyby
zderzanie kulek bylo zgodne z akcelerometrem.

No, ale do tego potrzebna jest rewizja systemu
animacji

* 13.01.2025

No to wczoraj cos tam probowalismy zrobic zeby miec
refleksje, ale nasz modul nie dziala z jakichs
niezrozumialych powodow.

(Warto moze dodac, ze refleksja nie jest tutaj jakos
super-wazna)

Natomiast super-wazne jest to, co zrobimy. Ale zacznijmy
dzisiaj moze od czegos prostego i lekkiego, czyli od tego,
zeby podwojne klikniecie nie powoodowalo zerowania
pionowej skladowej transformacji.

No dobra, to mamy. Jednak znow powraca pytanie co robimy:
- suwak?
- widget z kulkami?
- poprawke w kilkaniu na pustych dokumentach?
- poprawke przy sledzeniu kursora?

A na co mamy ochote?

w sumie to na ten rok najwazniejsze tematy to:
- wydanie do f-droid
- ksiazka
- eksplorator modulow
- algorytm a* i rozne zabawy ze stepperem

No i pieknie. I co teraz?

Moze te kulki?

Czy refleksja?

No bo jezeli idzie o refleksje, to fajnie by bylo
zrobic taki "wrapper":

#+BEGIN_SRC scheme
  (define-object (ObjectWrapper object ::java.lang.Object)::Enchanted
...
Moze raczej cos w stylu

#+BEGIN_SRC scheme
  (define (ObjectWrapper object::java.lang.Object)
    ::Enchanted
    (let* ((class (object:getClass)))
      (bordered
       (below
	(bordered
	 (Capiton (class:getName))
	 (GridColumns
...
No i jaki teraz jest dalszy plan?

Jeden pomysl to jakos zintegrowac API do refkeksji
(zeby $lookup$ dzialal normalnie na androidzie)

#+BEGIN_SRC scheme

  (define-syntax-rule ($lookup$ object name)
    (if (is name in (property-names object))
	(slot-ref object name)
	(lambda args
...
To co? Suwaki bysmy zrobili?

No, ale najpierw by pewnie trzeba bylo zrefaktoryzowac
sposob uzywania overlaya.

* 09.01.2025

No i dobra, i co teraz chcemy robic?

Mamy kilka mozliwosci:
- z jednej strony skupic sie na ksiazce
- z drugiej, pracowac nad widgetem do ksiazki
  (tym z kulkami)
- z trzeciej to mozemy wrocic do tego naszego calego
  "usability"

Wczoraj udalo sie wymyslic "owoc nisko wiszacy"
dla zmian w edytorze, czyli:
- czyscic ekran na szaro
- przed wyrysowaniem wszystkiego wyrenderowac
  "bialy" prostokat w rozmiarze dokumentu
- i jeszcze ewentualnie zrobic tak, zeby jakosc
  wyswietlania zalezala od zooma

(a na wyzszym poziomie, warto by bylo podzielic
sobie dni na takie, w ktore pracujemy nad widgetem,
takie, w ktore pracujemy nad ksiazka, i takie,
w ktore dopracowujemy edytor, zeby poruszac sie
rownolegle na wszystkich frontach)

No to w androidzie mamy to mniej wiecej zrobione;
teraz pozostaje nam przeniesc to na desktop, bo
w terminalu chyba tego nie chcemy?

Ok, na desktopie tez jest.

No to teraz co dalej?

Do wyboru mamy:
- dalsze prace nad zapisywaniem i przywracaniem
  workspace'u
- prace nad wyszukiwaniem
- prace nad widgetem z kulkami
- prace nad ksiazka
- prace nad systemem dla ksiazki

Wydaje sie, ze z perspektywy "uruchomienia
sprzedazy" nieodzowne elementy to z jednej strony
system do wyswietlania ksiazek, a z drugiej
sama ksiazka.

Nalezaloby przeyslec, w jaki sposob w ogole
chcielibysmy te ksiazke reprezentowac, i w jaki
sposob chcemy ja wysiwetlac, oraz jakie
funkcje chcielibysmy udostepnac czytelnikowi
(zakladki, wyszukiwanie, transkluzja?, skakanie
po linkach, komentarze)

No to nasze zalozenie jest z grubsza takie, zeby
przegladarka ksiazki byla widgetem.

Chyba ze chcemy po prostu czytac ksiazke jako dokument
w GRASPie. Wtedy trzeba by bylo jakos poprawic te czcionki
od komentarzy, i samo wyswietlanie komentarzy jakos
rozbudowac o te wszystkie markdowny itp.,
i cos wykombinowac z dzieleniem dokumentu na sekcje,
ewentualnie strony.

Ale zanim nawet sie nam uda osiagnac takie bardziej
wypolerowane doswiadczenie, chcielibysmy ogarnac
jakiekolwiek doswiadczenie.

I moze ten system modulow okazalby sie pomocny do
nawigowania.

Czyli moze na razie przyjmijmy taki plan dzialania, ze:
- piszemy ksiazke
- pracujemy bad uzytecznoscia GRASPa (w tym a*
  i widok modulow)
- robimy widgety do ksiazki

I dopiero jak juz bedziemy mieli duzo ksiazki, to
zaczniemy czytnik i ogolnie sie martwic

* 07.01.2025

no dobra, to chociaz sprobujmy zrozumiec, skad
koncepcyjnie sie biora nasze problemy

po pierwsze, jak rysujemy 8-pix, to dostajemy
(wizualnie) dosc dobra aproksymacje kola;
prawdopodobnie na nic lepszego w terminalu
liczyc nie mozemy.

jednak w tym przypadku rozdzielczosc
rysowania jest w poziomie dwa, a w pionie
cztery razy wieksza, niz rozdzielczosc
klikania mysza

dlatego moze bysmy zdefiniowali w painterze
- =precise-resolution-sideways=
- =precise-resolution-downward=
i przemianowali =draw-circle!= oraz =fill-circle!=
na =precise-draw-circle!= i =precise-fill-circle!=
i na koniec - =Canvas= na =PreciseCanvas=

* 06.01.2025 (komputer)

To plan bylby z grubsza taki, zeby dodac do paintera
(page-width). Chociaz moze wystarczy przemnozyc
(space-width) przez 60?

* 05.01.2025 (komputer)

No, to dzis bysmy sprobowali zrobic renderowanie
kolek w kliencie terminalowym.

A moze nawet we wszystkich klientach?

#+BEGIN_SRC

5 szerokosci ~ 3 wysokosci

⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿

no to niby by bylo fajnie, tyle ze to jeszcze
bardziej komplikuje rzecz i tak juz skomplikowana,
bo jezeli idzie o liczbe punktow, to

⣿⣿ ma 8x8

czyli 2 szerokosci ~ 1 wysokosci

#+END_SRC

No dobra, to sie wydaje, ze jedyny problem mamy tak naprawde
z wymiarami, tzn. dobrze by bylo, zebysmy mieli w painterze
wartosc "page-width" wyrazona w jednostkach tegoz paintera
(i dla terminala to ma byc oczywiscie 60, dla klienta 
desktopowego 400, a dla androidowego?).

No dobra, tylko teraz byloby dobrze jakos zapewnic, zeby
jakos sensownie odwzorowywac wspolrzedne Canvasa w te nasze
piksele
...
No dobra, to teraz chcielibysmy stworzyc takie
"plotno", na ktorym moglibysmy rysowac zarowno
w klientach graficznych, jak i tekstowych
(bazujac na 8pix), czyli zeby miec funkcje
w rodzaju "draw-circle!"

* 04.01.2025 (telefon)

No dobra, to moze zajmijmy sie czyms innym, mianowicie
jak doprowadzic do sytuacji, w ktorej edytujemy GRASPa
w GRASPie.

I chyba najprosciej byloby zaczac od sytuacji, w ktorej
edytujemy GRASP w shared storage

* 03.01.2025 (komputer)

Plan bylby taki, zeby stworzyc w GRASPie 
=PhysicsScene2D=

* 03.01.2025 (telefon)

Mozemy sprobowac sobie przemyslec narracje do ksiazki
(bo ze do naprawy mamy gest ewaluacji na Androidzie,
to wiadomo)

No to tak: chcielibysmy opowiedziec o cechach, jakie
powinien miec idealny jezyk programowania.

* 01.01.2025

no to pomalu wracamy chyba na normalne tory.
sprobujmy moze najpierw naprawic gest ewaluacji
na edytorze androidowym.

warto by tez bylo dopracowac przegladarke pliku
na desktopie, bo te "rooty" cos nie dzialaja
na windowsie chyba

i ogolnie z tym klientem windowsowym jest chyba
problem z przerysowywaniem

* 22.12.2024 (komputer)

Skoro tak sie tutaj komputeryzujemy, to moze warto by
bylo obsluzyc rzeczy z klawiatury.


#+BEGIN_SRC

normalne prosie zaprosiło się
a prosię Cię nie zaprosiło

bo prosię się prosiło
by gadać wciąż na wynos

prosię się prosiło bo chciało dalej pić oliwo

aż się prosi że gada prosię

prosię gada bo wciąż zajada
...
Lower bound of Edwin B. Wilson (1927) score confidence interval
...
pick 0.95 confidence to have a 95% chance that your lower
bound is correct, 0.975 to have a 97.5% chance, etc.
The z-score in this function never changes, so if you don’t
have a statistics package handy or if performance is an
issue you can always hard-code a value here for z.
(Use 1.96 for a confidence level of 0.95.)

* 20.12.2024

to bysmy tak zrobili, zeby =file-list= zamiast katalogu
przyjmowalo =(sequence-of java.io.File)= (ktora juz
na wstepie powinna byc posortowana)

Czyli tak: na razie mamy
...
#+BEGIN_SRC scheme
  (define-class (ParentDirectory directory::String)
...
to teraz nasze wywolania wygladaja tak:
...
a docelowo bysmy chcieli raczej cos w stylu
...
#+BEGIN_SRC
  ...
  "java.lang.NullPointerException: Attempt to invoke interface method 'int java.util.List.size()' on a null object reference
	at editor.input.document$Mneditor.fileList(document-editor.scm:168)
	at editor.input.document$Mneditor.openFileBrowser(document-editor.scm:221)
	at editor.input.document$Mneditor$frame2.lambda10$V(document-editor.scm:318)
	at editor.input.document$Mneditor$frame2.lambda10$check(document-editor.scm:306)
	...
	at io.github.grasp.GRASP.withPermissions(grasp-android.scm:1757)
	at io.github.grasp.GRASP.withPermission(grasp-android.scm:1761)
	at io.github.grasp.GRASP.withReadPermission(grasp-android.scm:1765)
	at editor.input.document$Mneditor$frame1.lambda9$V(document-editor.scm:306)
	at editor.input.document$Mneditor$frame1.lambda9$check(document-editor.scm:303)
	at java.lang.reflect.Method.invoke(Native Method)
	at gnu.mapping.CallContext$ReflectMethodHandle.invokeExact(CallContext.java:726)
	at gnu.mapping.CallContext.runUntilValue(CallContext.java:656)
	at gnu.mapping.Procedure.apply4(Procedure.java:174)
	at editor.types.extensions.Link.tap$Ex(widgets.scm:208)
	at editor.types.extensions.ColumnGrid.lambda8(widgets.scm:110)
	at editor.types.extensions.ColumnGrid.lambda8$check(widgets.scm:108)
	at java.lang.reflect.Method.invoke(Native Method)
	at gnu.mapping.CallContext$ReflectMethodHandle.invokeExact(CallContext.java:726)
	at gnu.mapping.CallContext.runUntilValue(CallContext.java:656)
	at editor.types.extensions.ColumnGrid.propagate(widgets.scm:73)
	at editor.types.extensions.ColumnGrid.tap$Ex(widgets.scm:105)
	at editor.input.PopUp.lambda8(pop-ups.scm:97)
	at editor.input.PopUp.lambda8$check(pop-ups.scm:95)
	at java.lang.reflect.Method.invoke(Native Method)
	at gnu.mapping.CallContext$ReflectMethodHandle.invokeExact(CallContext.java:726)
	at gnu.mapping.CallContext.runUntilValue(CallContext.java:656)
	at gnu.mapping.Procedure.apply4(Procedure.java:174)
	at editor.input.pop$Mnups.popUpAction$check(pop-ups.scm:54)
	at java.lang.reflect.Method.invoke(Native Method)
	at gnu.mapping.CallContext$ReflectMethodHandle.invokeExact(CallContext.java:726)
	at gnu.mapping.CallContext.runUntilValue(CallContext.java:656)
	at editor.input.PopUp.tap$Ex(pop-ups.scm:99)
	at editor.input.Overlay.lambda3(screen.scm:86)
	at editor.input.Overlay.lambda3$check(screen.scm:85)
	at java.lang.reflect.Method.invoke(Native Method)
	at gnu.mapping.CallContext$ReflectMethodHandle.invokeExact(CallContext.java:726)
	at gnu.mapping.CallContext.runUntilValue(CallContext.java:656)
	at gnu.mapping.Procedure.apply1(Procedure.java:154)
	at gnu.kawa.functions.ApplyToArgs.apply2(ApplyToArgs.java:52)
	at utils.functions.any(functions.scm:168)
	at editor.input.Overlay.tap$Ex(screen.scm:87)
	at editor.input.ActualScreen.tap$Ex(screen.scm:248)
	at editor.input.TouchEventProcessor.lambda2(touch-event-processor.scm:123)
	at editor.input.TouchEventProcessor.lambda2$check(touch-event-processor.scm:121)
	at java.lang.reflect.Method.invoke(Native Method)
	at gnu.mapping.CallContext$ReflectMethodHandle.invokeExact(CallContext.java:726)
	at gnu.mapping.CallContext.runUntilValue(CallContext.java:656)
	at gnu.mapping.Procedure.apply0(Procedure.java:148)
	at io.github.grasp.io$Dtgithub$Dtgrasp$DtGRASP$0.run(grasp-android.scm:199)
...
To dzis sprobujemy zrobic to menu do wyboru systemu plikow.
A powinno ono dzialac podobnie do zwyklego wyboru plikow.

No to moze warto by bylo opisac, jak aktualnie zwykly
wybor plikow dziala.
...
No dobra, czyli tak: ciskamy "Open..." albo
"Save as...", i w efekcie otrzymujemy wywolanie
=save-file-browser= albo =open-file-browser=,
do ktorych przekazujemy odpowiedni katalog.

W rezultacie - za pomoca =file-list= - do sceny zostaja
dodane obiekty, zawierajace =ColumnGrid= z taka przypadloscia,
ze jezeli kliknie sie na guzik wskazujacy na katalog,
to ponownie zostanie wywolany =open-file-browser=
albo =save-file-browser=, z przekazanym tym katalogiem.

I teraz tak: klikniecie na ikone katalogu spowoduje
ponowne wywolanie odpowiednio =open-file-browser=
badz =save-file-browser=

Natomiast my bysmy chcieli w tym wszystkim, zeby
wcisniecie tego guzika u gory otwieralo nam
=file-system-switcher=


* 18.12.2024

No dobra, to mamy sobie juz zaimplementowane
=file-system-roots=. Tyle ze wydaje sie, ze
z perspektywy istniejacej przegladarki plikow
- zeby nie musiec zbyt wiele zmieniac - nalezaloby
raczej zrobic hasz-mape, o np. taka:

#+BEGIN_SRC scheme
(define-mapping (file-name path ::string)::string
  path)
#+END_SRC
...
i wtedy by sobie poszczegolne klienty po prostu override'owaly

#+BEGIN_SRC scheme
(set! (file-display-name ...) "<Internal Storage>")
(set! (file-display-name ...) "<External Storage>")
#+END_SRC

itd.

I teraz kolejny krok to dodanie do przrgladarki pliku
wyboru "korzenia"

Ale zanim to nastapi, warto nieco pouzalac sie nad soba.

Ach.

No dobra.

Na pewno zdolamy przebudowac przegladarke, i na pewno
/jakos to bedzie/, ale rzecz wyglada tak, ze mamy jeszcze
w tym roku 6 dni commitowych:
- jutro (czwartek)
- poniedzialek
- wtorek (wigila)
- sroda (pierwszy dzien swiat)
- piatek
- sobota
(a w niedziele ma byc kotlet)

W miare mozliwosci chcielibysmy miec
juz cos uzywalnego, zeby nowy rok zaczac
probujac wepchnac GRASPa do f-droid.

W zwiazku z tym nalezaloby:
- napisac "contribution guide"
- przesledzic zrodla wszystkich zalaczonych
  zasobow (czcionki, obrazki, sloiki)
- zmodyfikowac skrypt tak zeby zamiast
  io.github.grasp miec io.github.panicz
- zrobic zgloszenie do F-droid
- doprowadzic kod do takiego stanu, zeby
  zminimalizowac dziwne zachowania
- stworzyc road-mape z planem wydawniczym
- zrobic jakis system do dokumentacji
  (oraz dokumentacje)


* 16.12.2024

Chyba jednak =file-system-roots= powinny przynalezec
do =Keeper='a, bo musza byc widoczne w implementacji

No dobra, to cos tam juz dziala, ale cos wciaz nie dziala :/

Tzn. jak za pierwszym razem wcisniemy =(ctrl h)=, to jest prawie OK,
ale za drugim razem to juz nie

* 12.12.2024

Chcielibysmy stworzyc nowy typ w =elements=,
moze
...
No dobra, ale poki co to moze lepiej byloby zrobic tak, zeby
przy zapisywaniu pliku ustawiac =document:source=.

* 11.12.2024

No dobra, to moze dzisiaj sprobujmy zrobic tak, zeby:
- po zapisaniu pliku a dysku (jako "save as") aktualizowac
  ich "source"
- otwierac przegladarke w tym folderze, w ktorym
  znajduje sie biezacy plik

No to to juz mamy. To teraz moze bysmy sprobowali
zrobic liste dostepnych systemow plikow

Jest sobie statyczna metoda =java.io.File:listRoots=,
i to z niej bysmy korzystali na JVM. A na Androidzie?

=getFilesDir=, =getCacheDir=, =getExternalFilesDir=,
=getExternalCacheDir=,
=android.os.Environment:getExternalStorageDirectory=

No i OK. Ale jak to nazwiemy?!

"file-system-roots"!

* 10.12.2024

To teraz zrobmy tak, zeby:
- "save as" powodowalo zmiane pola "source" w dokumencie
- przegladarka sie otwierala w folderze, zawierajacym plik
- sugestia nazwy pliku pochodzila od nazwy biezacego plku
- jezeli plik docelowy istnieje i jest rozny od pliku
  zrodlowego, to zrobic pop-up z potwierdzeniem

dalej bysmy zrobili tak, zeby:
- przestrzen robocza sama sie zapisywala i przywracala
- dalo sie wybierac litere dysku/rodzaj storage'u
  w przegladarce plikow

* 09.12.2024

no to tak:
jak otwieramy dokument, to to sie dzieje w taki sposob
(z perspektywy =open-file-browser=
...
wydaje sie zatem, ze analogicznie bysmy chcieli miec metode
=editor:new-file= analogiczna do =editor:load-file=, ktora jest
zaimplementowana tak oto:
...
gdzie =open-document-file= jest zaimplementowana jako
...
#+BEGIN_SRC scheme
  ...
  (call-with-input-string ""
      (lambda (port)
	(Document (parse port) (java.lang.LocalDateTime:now))))

#+END_SRC

Ok, mamy to z grubsza. Teraz jeszczd zrobic tak, zeby:
- jak sie zrobi "Save as", to zeby domyslnie pojawiala sie
  podpowiedz z nazwa biezacego pliku
- przegladarka otwierala sie w folderze, w ktorym
  znajduje sie biezacy plik

* 08.12.2024

No dobra, to bysmy sie moze sprobowali zajac tym przyciskiem 'new'.

Co on musi robic?
- tworzyc nowy dokument, oczywiscie.

* 07.12.2024

No dobra, to teraz zakladajac, ze chcemy dalej robic
advent of code, warto by bylo skupic sie na ficzerach,
ktore nam to ulatwia.

W szczegolnosci bysmy zaimplementowali przycisk New

* 06.12.2024

No dobra. Jeden bug zaraportowany (i jak na razie
bez odpowiedzi).

To teraz moze bysmy sie zajeli tym drugim bugiem

#+BEGIN_SRC
#<ERROR Assertion failed:  (= index 0)>
at kawa.lib.exceptions.error(exceptions.scm:105)
at editor.types.primitive.cellIndex(primitive.scm:399)
at editor.types.cons.partAt(primitive.scm:319)
at editor.input.DocumentEditor.lambda39$V(document-editor.scm:968)
at editor.input.DocumentEditor.lambda39$check(document-editor.scm:959)
at java.lang.reflect.Method.invoke(Native Method)
at gnu.mapping.CallContext$ReflectMethodHandle.invokeExact(CallContext.java:726)
at gnu.mapping.CallContext.runUntilValue(CallContext.java:656)
at gnu.mapping.Procedure.applyN(Procedure.java:142)
at gnu.mapping.Values.call_with(Values.java:107)
at gnu.kawa.functions.ApplyWithValues.applyWithValues(ApplyWithValues.java:23)
at gnu.kawa.functions.ApplyWithValues.apply2(ApplyWithValues.java:29)
at editor.input.DocumentEditor.lambda38$V(document-editor.scm:959)
at editor.input.DocumentEditor.lambda38$check(document-editor.scm:959)
at java.lang.reflect.Method.invoke(Native Method)
at gnu.mapping.CallContext$ReflectMethodHandle.invokeExact(CallContext.java:726)
at gnu.mapping.CallContext.runUntilValue(CallContext.java:656)
at gnu.mapping.Procedure.apply2(Procedure.java:160)
at gnu.mapping.Values$Values2.call_with(Values.java:376)
at gnu.kawa.functions.ApplyWithValues.applyWithValues(ApplyWithValues.java:23)
at gnu.kawa.functions.ApplyWithValues.apply2(ApplyWithValues.java:29)
at editor.input.DocumentEditor.secondPress$Ex(document-editor.scm:959)
at editor.input.ActualScreen.secondPress$Ex(screen.scm:284)
at editor.input.TouchEventProcessor.move$Ex(touch-event-processor.scm:70)
at io.github.grasp.GRASP.onTouchEvent(grasp-android.scm:1833)
at android.app.Activity.dispatchTouchEvent(Activity.java:4198)
...
at android.view.ViewRootImpl$ViewPostImeInputStage.processPointerEvent(ViewRootImpl.java:6606)
at android.view.ViewRootImpl$ViewPostImeInputStage.onProcess(ViewRootImpl.java:6388)
at android.view.ViewRootImpl$InputStage.deliver(ViewRootImpl.java:5862)
at android.view.ViewRootImpl$InputStage.onDeliverToNext(ViewRootImpl.java:5919)
at android.view.ViewRootImpl$InputStage.forward(ViewRootImpl.java:5885)
at android.view.ViewRootImpl$AsyncInputStage.forward(ViewRootImpl.java:6050)
at android.view.ViewRootImpl$InputStage.apply(ViewRootImpl.java:5893)
at android.view.ViewRootImpl$AsyncInputStage.apply(ViewRootImpl.java:6107)
at android.view.ViewRootImpl$InputStage.deliver(ViewRootImpl.java:5866)
at android.view.ViewRootImpl$InputStage.onDeliverToNext(ViewRootImpl.java:5919)
at android.view.ViewRootImpl$InputStage.forward(ViewRootImpl.java:5885)
at android.view.ViewRootImpl$InputStage.apply(ViewRootImpl.java:5893)
at android.view.ViewRootImpl$InputStage.deliver(ViewRootImpl.java:5866)
at android.view.ViewRootImpl.deliverInputEvent(ViewRootImpl.java:8920)
at android.view.ViewRootImpl.doProcessInputEvents(ViewRootImpl.java:8860)
at android.view.ViewRootImpl.enqueueInputEvent(ViewRootImpl.java:8817)
at android.view.ViewRootImpl$WindowInputEventReceiver.onInputEvent(ViewRootImpl.java:9052)
at android.view.InputEventReceiver.dispatchInputEvent(InputEventReceiver.java:259)
at android.view.InputEventReceiver.nativeConsumeBatchedInputEvents(Native Method)
at android.view.InputEventReceiver.consumeBatchedInputEvents(InputEventReceiver.java:239)
at android.view.ViewRootImpl.doConsumeBatchedInput(ViewRootImpl.java:9005)
...
#+BEGIN_SRC
java.lang.ClassCastException: editor.types.EmptyListProxy cannot be cast to gnu.lists.Pair
at editor.types.primitive.cellIndex(primitive.scm:411)
at editor.types.cons.partAt(primitive.scm:319)
at editor.input.DocumentEditor.lambda36$V(document-editor.scm:853)
at editor.input.DocumentEditor.lambda36$check(document-editor.scm:844)
at java.lang.reflect.Method.invoke(Native Method)
at gnu.mapping.CallContext$ReflectMethodHandle.invokeExact(CallContext.java:726)
at gnu.mapping.CallContext.runUntilValue(CallContext.java:656)
at gnu.mapping.Procedure.applyN(Procedure.java:142)
...
at gnu.kawa.functions.ApplyWithValues.applyWithValues(ApplyWithValues.java:23)
at gnu.kawa.functions.ApplyWithValues.apply2(ApplyWithValues.java:29)
at editor.input.DocumentEditor.lambda35$V(document-editor.scm:844)
at editor.input.DocumentEditor.lambda35$check(document-editor.scm:844)
at java.lang.reflect.Method.invoke(Native Method)
at gnu.mapping.CallContext$ReflectMethodHandle.invokeExact(CallContext.java:726)
at gnu.mapping.CallContext.runUntilValue(CallContext.java:656)
at gnu.mapping.Procedure.apply2(Procedure.java:160)
...
at gnu.kawa.functions.ApplyWithValues.applyWithValues(ApplyWithValues.java:23)
at gnu.kawa.functions.ApplyWithValues.apply2(ApplyWithValues.java:29)
at editor.input.DocumentEditor.press$Ex(document-editor.scm:844)
at editor.input.ActualScreen.press$Ex(screen.scm:258)
at editor.input.TouchEventProcessor.move$Ex(touch-event-processor.scm:71)
at io.github.grasp.GRASP.onTouchEvent(grasp-android.scm:1833)
at android.app.Activity.dispatchTouchEvent(Activity.java:4198)
at com.android.internal.policy.DecorView.dispatchTouchEvent(DecorView.java:460)
at android.view.View.dispatchPointerEvent(View.java:14837)
...
mamy jeszcze jeden problem: forma =and-let*= nie jest
prawidlowo ewaluowana [i forma =match= tez nie jest].

Moze problemem jest uzycie slowa =match=?

(jezeli tak, to =match/evaluated= powinno mormalbie dzialac)

Trzeba bedzie moze zapytac Pera Bothnera - i w tym celu
dobrze by bylo zrobic tak, zeby funkcja obslugujaca
wyjatek mogla opcjonalnie zachowac dane do schowka
...
poprawic blad generowany z =document-editor:959/968=
=(EmptyListProxy cannot be cast to gnu.lists.Pair)=

* 03.12.2024

No dobra, to mamy teraz niesamowita okazje do wprowadzania
praktycznych usprawnien w GRASPie:

po pierwsze, chcielibysmy moze na poczatek, zeby dwukrotne
tapniecie robilo jedna z nastepujacych rzeczy:
- przy tapnieciu lewego nawiasu zamykalo albo otwieralo
  wyrazenie (ale tego na razie nie robimy)
- przy tapnieciu prawego nawiasu maksymalizowalo wyrazenie
  (ale tego tez na razie nie robimy
- przy tapnieciu pustego obszaru gdzekolwiek, jezeli mamy
  rotacje, chcemy anulowac te rotacje
- przy tapnieciu pustego obszaru wewnatrz pudelka chcemy
  ustawic taka transformacje, zeby caly wiersz, w ktorym
  znajduje sie to pudelko, miescil sie na szerokosc w
  biezacym edytorze

I to jest jedna sprawa. Druga natomiast jest taka, warto
sprobowac rozwazyc, dlaczego ewaluator na androidzie tak
dziwacznie dziala, i czy da sie cos z tym zrobic (w
najgorszym razie po prostu napiszemy nasz wlasny?).

Trzecia zas sprawa jest taka, ze dobrze byloby usprawnic
edycje z klawiatury (w tym w szczegolnosci, zeby dalo sie
otwierac okna itd)

Tak czy inaczej, na razie po prostu probujmy w GRASPie
realizowac po kolei zadania z Advent of Code (bez
pospiechu), i obserwujmy, czego nam brakuje.

No to tak, jezeli idzie o "usability", to chcemy:
- zeby po wyjsciu z programu i ponownym otwarciu
  byl przywracany obszar roboczy
- zeby "save as" otwieralo lokalizacje biezacego
  dokumentu
- zeby dalo sie sterowac tym wszystkim z klawiatury

No ale wszystko po kolei.

Czyli tak: bierzemy sobie element top-levelowy, czyli taki,
na ktory indeks ma postac =`(,x ,1)=, tj.

#+BEGIN_SRC scheme
  (first-cell (lambda (cell)
	       (and-let* ((`(,_ 1) cell))))
	      path)

  (first-cell (fn (`(,_ 1))) path)
#+END_SRC

No ale mniejsza. Bierzemy sobie ten element, jego pozycje
w dokumencie oraz rozmiar, i szukamy takiej skali, ze:
- nie moze byc wieksza, niz =(painter:max-document-scale)=
- nie moze byc mniejsza, niz =(painter:min-document-scale)=
- po jej zastosowaniu element zmiesci sie w szerokosci edytora,
  jego lewa i prawa krawedz beda odsuniete od krawedzi ekranu
  o =(painter:document-margin)=

* 02.12.2024

Wczoraj tosmy sie glownie zajmowali tym Advent of Code,
i jak na razie poleglismy - bo cos w tym ewaluatorze wydaje
sie nie dzialac tak, jak powinno.

No dobra, to teraz mamy dwie drogi.

Pierwsza jest taka, ze probujemy zrozumiec ten kod, ktory
mamy teraz napisany.

Druga jest taka, ze probujemy wymyslic model edycji,
ktory jest fundamentalnie zrozumialy (TeleGRASP?).

No bo jednak - co by nie mowic - te listy puste powoduja
problemy.

Wydaje sie jednak, ze tego, co teraz mamy,
nie mozemy wyrzucic do kosza (bo ewaluator),
a z drugiej strony przed proba przeprojektowania
istniejacego rozwiazania warto byloby sie zapoznac
z Interlispowym s-editem (czyli moze otworzyc ticket
na githubowym repo?)
