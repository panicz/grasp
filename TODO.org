...
(define-type (PopUp left: real := 0 top: real := 0
  ...
  implementing Pane
  with
  ((tap! finger::byte #;at x::real y::real)::boolean #f)
  ((press! finger::byte #;at x::real y::real)::boolean
   (let* ((painter ::Painter (the-painter))
	  ...
	  (right ::real
     (if (or (is 0 <= x < horizontal) ;>>
             (is 0 <= y < vertical) ;>>
	     (is (+ inner:width horizontal) < x) ;>
	     (is (+ inner:height vertical) < y)) ;>
	 (screen:dragging:add!
     ))
  ((move! finger::byte x::real y::real dx::real dy::real)
  ::boolean #f)
  ((release! finger::byte x::real y::real vx::real vy::real)
  ::boolean #f)
  ((second-press! finger::byte #;at x::real y::real)::boolean
  #f)
  ((double-tap! finger::byte x::real y::real)::boolean #f)
  ((long-press! finger::byte x::real y::real)::boolean #f)
