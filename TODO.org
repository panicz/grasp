...
(define-type (FileButton target: File
  ...
  with
  ((compareTo other::FileButton)
   (cond
     ((and (target:isDirectory) (not (other:target:isDirectory)))
      -1)
     ((and (not (target:isDirectory)) (other:target:isDirectory))
      +1)
     (else
  ...
  (let* ((filename ::String[] (directory:list))
    ...
    (for i from 0 to n
      (let* ((file (File directory (filename i)))
             (action (if (file:isDirectory)
	                 directory-action
			 file-action)))
        (set! (button (+ i 1)) (FileButton target: file
                                           action: action))))
    (Array:sort button)
    (let* ((scroll ::Scroll
